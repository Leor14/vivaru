...
Es el mismo patrón que tuvo el transporte del SRI en el módulo de [[billing]] —una interfaz con implementación intercambiable—, **aunque ese código ya no existe: lo fiscal salió del alcance el 19 de agosto de 2026 y el SRI se retiró entero de los dos ambientes** (ver [[integridad-financiera]]). Se deja la comparación porque el patrón sigue siendo el mismo; lo que no hay que hacer es ir a buscarlo. Y por el mismo motivo. El simulado no es un sustituto pobre sino la herramienta correcta para una parte del trabajo: el criterio del paso es que una respuesta malformada se rechace, y **al modelo real no se le puede pedir que se equivoque cuando conviene**.
