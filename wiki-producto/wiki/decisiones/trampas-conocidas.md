...
## Una bandera encendida puede dar a una puerta cerrada MÁS ARRIBA

**La tercera forma de «encender no era el arranque», y la más callada.** Las otras dos ya estaban documentadas: una bandera sobre una tabla vacía, y un front desplegado que no conocía la clave. Ésta es peor de diagnosticar porque **todo lo que se mira está bien**: código desplegado, bandera encendida y verificada por las tres vías (documento, catálogo del servidor, override por conjunto), y aun así no pasa nada. Pasó el 27 de agosto de 2026 con `producto-entrega-de-correo`: se disparó un recordatorio, la callable respondió `200`, la notificación in-app se creó, y `emailDeliveries` siguió en cero **sin un solo error en el log**. El freno estaba una capa por encima, en producto: `deliverResidentNotifications` corta en `index.ts:595` —`if (!copy.emailEnabled) return;`— y **las 13 claves del catálogo tienen `emailDefault: false` con cero overrides en los 8 conjuntos**. La regla: cuando una capacidad encendida no produce nada y **no hay error**, no buscar el fallo dentro de la capacidad — **recorrer hacia arriba las puertas que están ANTES**, y medir cada una contra el dato. El síntoma de una puerta cerrada aguas arriba y el de algo roto son idénticos: silencio. Ver [[banderas-funcionalidad]] y [[notificaciones-residentes]].

## Un secret de Cloud Functions v2 se clava a una VERSIÓN, no sigue a `latest`

Medido el 27 de agosto de 2026 en `resendWebhook`, cuyo `serviceConfig` decía `version: "1"`. Añadir una versión 2 con el valor bueno **no la alcanza**: la función sigue arrancando con la vieja. Y el síntoma —todo rechazado— es **idéntico al de una clave mal copiada**, así que se busca en el sitio equivocado. **Cambiar el valor de un secret obliga a redesplegar las funciones que lo usan**; el CLI lo ofrece solo («re-deploy the functions and destroy the stale version»), y hay que aceptar. Se comprueba a qué versión está clavada leyendo `secretEnvironmentVariables` de la función por la API, que no expone el valor. Ver [[correos-mensajeria]].

## Un endpoint de webhook NO se prueba con el navegador

Abrir la URL de un webhook en el navegador manda un `GET`, y un webhook bien escrito responde **`405 method not allowed`**. Eso es su conducta correcta, no una avería — pero se lee como «la liga no funciona» y frena el trabajo. Pasó el 27 de agosto de 2026 con `resendWebhook`, y costó una vuelta entera: el endpoint **ya estaba registrado en Resend** (el `whsec_` solo existe si lo creaste allá). Para probarlo de verdad: `curl -X POST` —debe dar `401`, que significa «te oí, pero no vienes firmado»— o el «send test event» del propio proveedor. La prueba de que la firma verifica es un **`200` en el log en vez de un `401`**, y eso solo lo puede producir el proveedor.

## Las credenciales de `gcloud` son DOS y caducan por separado — probar antes de pedir

`gcloud auth print-access-token` (la del CLI, de `gcloud auth login`) y `gcloud auth application-default print-access-token` (la ADC) son **independientes**. El 27 de agosto de 2026 la primera murió a mitad de sesión con «Reauthentication failed. cannot prompt during non-interactive execution» mientras **la ADC siguió viva y sirvió para todo**: leer functions, reglas, índices, secretos y rollouts por API. El síntoma engaña porque una llamada que usaba la primera devolvió un JSON de error de autenticación que **parece que el recurso no existe**. Regla: cuando la duda es «¿tengo credencial?», **probar la ADC con una lectura real y mirar el error**, no deducirlo de otro comando ni pedirle al usuario que reautentique algo que funciona. Son tres en total con `firebase login --reauth`. Y la comprobación de qué hay desplegado se hace **contra el servicio, no contra una rama** — ver [[roadmap-tecnico]] y el bloque de orden de despliegue de `CLAUDE.md`.
