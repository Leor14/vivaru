...
| **Responsable** | David |
| **Estado** | 🟢 **LAS TRES ENTREGAS EN PRODUCCIÓN** (4 sep 2026, `3f0de0a`), con la bandera **ENCENDIDA en los diez conjuntos de producción** desde el 4 sep: ocho por el valor global y dos —Las Playas y Santa María— por un override que dice lo mismo (esta celda decía «apagada» y se quedó atrás). En staging, solo en Santa María. *Medido el 12 sep 2026 resolviendo conjunto por conjunto con `functions/lib/feature-flags.js`.* `R8` cerrado. Escrita tras medir el código y los datos de producción; **David cerró las dos preguntas abiertas y aceptó `G1` vacía el mismo día**. Ver §14 |
