...
| **Responsable** | David |
| **Estado** | **EN PRODUCCIÓN, con la bandera encendida SOLO en `tenant-santa-maria`** (30 ago 2026, versión 0.6). Desplegada entera la noche del 29–30: reglas (ruleset vivo 0 diff), functions (87/87 frescas medidas), front (rollout `build-2026-08-29-015` desde `b70c357`; el SW v2 servido con la config de `hogaru-1`), y el canario verificado resolviendo con el compilado. Validada antes en staging con un iPhone real (ciclo entero, §14). **Pendiente: validar en producción con cuenta real de prod, un Android, y la entrega 2.** NO productiva: G5 espera un residente de verdad |
