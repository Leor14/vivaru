...
| **Responsable** | David |
| **Estado** | ✅ **ENTREGA 2 EN PRODUCCIÓN** (11 sep 2026 por la noche, `fde9931`, §16), después de `FIX-004`: sus seis functions a las 00:50 UTC del 12 —`setTenantAdminAccess` **creada**, con `run.invoker = allUsers` y CORS desde `www` comprobados— y el front con `build-2026-09-12-003`. La lista de seis se comprobó contra el código: el cierre transitivo del diff, **sin comentarios**, da exactamente esas más las dos de `FIX-004`. **No se tocó ninguna cuenta de producción**: dar un segundo conjunto a alguien es dato de producción y lo decide David — y con eso **CA1 por fin se podría observar**. **Antes: EN STAGING Y VISTA EN PANTALLA** (11 sep 2026, commit `6da1ae0`, §16). Recorrida con una cuenta desechable y la sesión de superadmin: la lista da cada admin **una vez con todos sus conjuntos** (CA18); crear con el correo de un residente enseña el **aviso con su conjunto y su unidad** (CA20) y lo convierte guardando su membresía de residente; añadir un conjunto va directo y marca las dos como compartidas (CA14); `/admin/users` de **El Nogal** —donde su `users.tenantId` NO apunta— la lista con «Lo gestiona Vivaru» (E2-R9, CA23); quitarle todo la **devuelve a residente de su unidad**, con el claim reapuntado y las sesiones cerradas (CA21, E2-R7); y cada paso quedó en `auditLogs` de su conjunto (E2-R11). **Hueco de datos visto, solo en staging:** las membresías sembradas por script no llevan `compartida`, así que la cuenta de David sale sin la marca; el servidor la protege igual (E2-R8). **Antes:** construida en la rama `trabajo/admins-multiconjunto`: el superadmin da acceso a varios conjuntos por persona desde la pestaña Admins. Servidor (`setTenantAdminAccess` con modo `simular`, alta con varios conjuntos, edición que ya no muda, E2-R8 en las tres acciones de `/admin/users`), consola y `/admin/users`. 35 pruebas de servidor con **ocho falsaciones** y 11 de pantalla con tres. Va **después** de `FIX-004`. **Y esta celda estaba mal en la bandera:** `producto-multiconjunto` está **ENCENDIDA por el valor global en los diez conjuntos de producción**, medido el 11 sep con `resolveFeatureFlag`. Decía «apagada y su documento no existe». **EN PRODUCCIÓN** desde la tarde del 25 de agosto de 2026 (`e41affa`) — versión 1.2. Validada antes en pantalla contra staging. §11.2 completa (eran **dieciocho** sitios, no once) · sesión con varias membresías, `lastActiveTenantId` y **selector** construidos y verificados por navegador con una cuenta de seis conjuntos: CA2, CA3, CA4, CA5, CA10 y **un cobro real en el segundo conjunto**. El **paso 3 está hecho** (entidad `managementCompanies`, sus dos callables de superadmin y `/superadmin/administradoras`) y **la medición de `storage.rules` se cerró**: las reglas entre servicios no funcionan en el servicio real, así que el claim **sigue** al conjunto activo — ver §11.3. **Desplegado y verificado contra su fuente**, con el radio del cambio de autoridad medido en **0** con el predicado real. **La bandera sigue APAGADA y su documento no existe** en producción, y **CA1 sigue sin observarse**: nadie tiene dos membresías, así que está cumplido por construcción y no visto |
...
**Estado: EN PRODUCCIÓN** (11 sep 2026 por la noche, `fde9931`; antes en staging con `6da1ae0` y vista
en pantalla). **Fue DESPUÉS de `PRD-V-FIX-004`**, que cierra el camino por el que
