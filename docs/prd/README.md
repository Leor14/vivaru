...
| [PRD-V-FLOW-002 — Anticipos y aplicación del pago a varios cargos](funcionales/PRD-V-FLOW-002-anticipos-y-aplicacion-del-pago.md) | **NO lista para desarrollo** (1.1, 23 ago 2026 tarde) — desbloqueada, pero la PRD se quedó atrás | Tanda 2. Tres defectos medidos en `functions/src/payments.ts`: **el sobrepago se evapora** (`balance = max(0, cobrado − pagado)`, y el excedente se contabiliza como ingreso sin dejar saldo a favor), **un pago solo aplica a un cargo**, y **el asiento se escribe con `bankAccountId: null` fijo**. El cambio de firma de `aplicarPago` es **aditivo** para no romper producción. Cerradas: el anticipo es ingreso del mes en que entra, **en su propia línea** · la imputación la decide la administración. **Tres correcciones del 23 ago, marcadas dentro de la PRD:** (1) **la trampa cambió de forma** — la exclusión ya no mira la categoría sino el ORIGEN, así que se pasa de omisión a **herencia**: hoy se cae por defecto si el asiento del anticipo hereda `sourceType: "billingStatement"` de `aplicarPago`, y **hay que actuar para EVITARLA**; (2) **R4 es cierta y NO basta** — cruzar sube `paymentAmount` y con él `cuotaIncome`, así que el doble conteo **no pasa por el libro** y **CA6 pasaría en verde con el estado financiero mal**; (3) **D-C nombra uno de los DOS `bankAccountId: null`** — el de `revertirPago` no aparecía. Los tres defectos siguen vigentes; sus líneas se movieron |
...
| 6 | **`FLOW-002` · anticipos** | Desbloqueada: `PLAT-003` aterrizó el 23 ago. **Corregir la PRD antes de construir** |
