...
**Las once funcionales, por estado:** 2 productivas · **6 en producción con sus banderas
apagadas** (`PLAT-001`, `PLAT-002` e1, `PLAT-003`, `FEAT-003`, `FIX-001` e1 y `FLOW-002`) ·
3 listas para desarrollo sin empezar (`FLOW-001`, `FEAT-004`, `FLOW-003`).
...
| # | PRD | Por qué aquí |
|---|---|---|
| 1 | **`PLAT-002` · auditoría de las once callables** ✅ **en producción** (23 ago) | Va **sola y primero**. Es el único cambio del lote que **no se revierte con una bandera** |
| 2 | **`FIX-001` · entrega 1** ✅ **en producción** (23 ago, bandera apagada) | Corrección de una regla de negocio que hoy solo vive en el cliente. **No cambia nada visible** |
| 3 | **`PLAT-001` · copropiedad** ✅ **MVP en producción** (23 ago, bandera apagada) | Base de la que dependen catorce candidatos |
| 4 | **`FEAT-003` · proveedores** ✅ **MVP en producción** (23 ago, bandera apagada) | Independiente de todo lo demás |

#### Ola B — contabilidad

| # | PRD | Por qué en este orden |
|---|---|---|
| 5 | **`PLAT-003` · plan de cuentas** ✅ **en producción** (23 ago, con las banderas apagadas) | **Antes que `FLOW-002`.** Si va después, `FLOW-002` añade el valor `"anticipo"` a un enum que `PLAT-003` sustituye acto seguido |
| 6 | **`FLOW-002` · anticipos** ✅ **en producción** (24 ago) | **PRD en v1.5.** `producto-anticipos` encendida solo en el conjunto de demostración. **Sin construir: §9/CA13, CF8 y `personId`** |
...
>
> **Estado al 24 de agosto de 2026: las DOS aterrizaron.** `PLAT-003` entró el 23 y `FLOW-002`
> el 24, las dos en producción con sus banderas apagadas, así que **no queda nada en vuelo sobre
> `aplicarPago`**. Se conserva la regla —no pueden estar en vuelo a la vez— porque vuelve a
