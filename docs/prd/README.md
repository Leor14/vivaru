...
| [PRD-V-FLOW-008 — Cuentas por pagar con calendario de cuotas](funcionales/PRD-V-FLOW-008-cuentas-por-pagar-con-calendario-de-cuotas.md) | **Discovery** (4 sep 2026) — `G0`, `G2`, `G4`, `G5` y `G6` superadas; **`G1` NO** (cero clientes) y **`G3` en AMARILLO**: toca el núcleo del estado financiero y **una cifra que YA está en producción**, y endurece una regla, lo que **invierte el orden de despliegue** (functions → front → reglas). 14 criterios, **seis deben fallar**, uno es la falsación. Dos `TBD` para David | **§3.2 de la sesión con la administradora**, y NO de los candidatos: ninguno de los diez de la categoría `E` lo cubre — el hueco salió de **escuchar**, no de navegar. «La del seguro, yo pago en **once cuotas**». Medir corrigió el tamaño en las dos direcciones: no falta solo el calendario, **tampoco existe el pago parcial de un egreso** y `ledgerEntryId` es **singular**, así que once cuotas no caben. Y destapó lo que más importa: **esta ficha ROMPE la deuda a proveedores de `FLOW-007`** —que suma el importe COMPLETO de todo egreso en `registrado`— y por eso la corrección va dentro. Baseline: **32 de 52 egresos sin vencimiento**, y **el caso que la motiva ya está en la base aplanado** («Póliza de seguro del inmueble (trimestral)», un solo pago) |
