...
| [PRD-V-FEAT-006 — Unir varias columnas del archivo en un campo](funcionales/PRD-V-FEAT-006-unir-columnas-en-un-campo.md) | **Lista para PRD** (1 sep 2026) — `G0`–`G3` superadas, **`G1` NO**: la métrica está definida y `importRuns` está **vacío en producción**, así que no hay tráfico contra el que leerla. Lista para desarrollo; no podrá marcarse productiva hasta que alguien importe de verdad | Sale de la exploración de `AI-ONB-001` y **no lleva IA**. Hoy el mapeo es una columna → un campo, así que un padrón con «Nombres»+«Apellidos» o «Torre»+«Apto» **no tiene entrada ni a mano**: el producto solo sabe decir «júntalas en Excel». **El detector del caso ya está en producción** (`agrupacionSinUsar`/`unidadesQueSeFunden`, 1 sep): hoy bloquea, y aquí pasa a **ofrecer la unión**. **Enmienda el `RN-02` de `FEAT-002`** —dos columnas a un mismo campo pasa a estar permitido, pero solo por acto explícito (`RN-U3`)— y conserva la otra mitad. Sin colección, sin permiso, sin bandera y sin callable nuevos: la unión es transformación de lectura y el archivo sigue sin salir del navegador (§7 de `FEAT-002`). **Acotada a UNIR**; partir una celda y las plantillas con prefijo son Fase 2, con la consecuencia declarada de que Torre «1» + Apto «101» da `1-101` y no `T1-101` |
