...
> **no producen nada**, porque la tabla que alimentan está vacía en producción: `0` de `88` unidades
> con coeficiente, `0` proveedores, `0` cuentas del plan. Medido el 24 de agosto resolviendo las
> banderas con el código del servidor, no leyendo documentos. **Encender no es lo mismo que poner en
> uso**, y la columna de estado decía «apagada» de las tres, que era la época anterior.
...
| 2 | **`FIX-001` · entrega 1** ✅ **en producción, bandera APAGADA** (sigue apagada en los nueve al 24 ago — es el frente 3 y no es solo un interruptor) | Corrección de una regla de negocio que hoy solo vive en el cliente. **No cambia nada visible** |
