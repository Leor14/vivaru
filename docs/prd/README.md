...
| **`PRD-V-FLOW-005` — Autorizar la visita que llega sin avisar** | Frente encuadrado el 30 de agosto de 2026 (`PH-003`), con sus cinco decisiones ya tomadas. **No se escribe todavía por el criterio de cerrar frentes antes que abrirlos.** Necesita un estado nuevo —*pendiente de autorización*— y **romper la exclusividad entre `qr_full` y `registro_simple`**, que hoy impone el servidor |
| **`PRD-VAI-FEAT-001` — Cargar el padrón desde lo que el cliente ya tiene** | Es una de las cuatro de Drive, y su **alcance cambió el 30 de agosto**: incluye PDF y fotos por decisión de David, contra lo que argumentaba `docs/hoja-de-ruta-ia.md`. Al migrarla hay que reescribir ese alcance, no copiarlo. **La parte de duplicados NO es suya y no lleva IA**: se hace antes y sin modelo |
