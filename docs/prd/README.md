...
| [PRD-V-PLAT-005 — Notificaciones push al residente](funcionales/PRD-V-PLAT-005-notificaciones-push-al-residente.md) | **Borrador 0.1** (29 ago 2026). Tercer canal del sistema de notificaciones: el aviso que ya se escribe en `notifications` llega además al hub del teléfono, vía FCM/Web Push, sin app en las tiendas. Bandera `producto-notificaciones-push`, nace apagada. **0.2:** D1 y D2 cerradas —tope de 5 dispositivos, iconos generados del SVG (solo emblema, 192/512/180)—; y D-CONSOLA también: el par VAPID generado y las claves públicas cableadas en los dos `apphosting*.yaml`. **MVP CONSTRUIDO el 29 ago** — cuatro bancos en verde y cuatro falsaciones; **EN PRODUCCIÓN el 30 ago, bandera solo en Santa María** (patrón FLOW-004), tras validar el ciclo entero en staging con un iPhone real — registro, push con app cerrada, toque que navega, baja, re-alta. Cinco hallazgos del teléfono en §14. Falta Android y la validación en prod | Medido antes de escribir: `registerWebPush` existe con **cero consumidores**, sin SW ni manifest ni lado emisor; `notifications` tiene 507 docs en producción y `pushTokens` 0. El envío se engancha en el embudo único `createNotifications` (gemelo bueno: el correo en `deliverResidentNotifications`). En iOS exige añadir a pantalla de inicio — el riesgo es de adopción, no de ingeniería. `G5` se abre con el primer dispositivo real recibiendo |
...
| [PRD-V-PLAT-004 — Alcance del rol Consejo](funcionales/PRD-V-PLAT-004-alcance-del-rol-consejo.md) | **Discovery — `G5` abierta** (4 sep 2026). Escrita después de medir, y la medición cambió la ficha: `committee` **no se puede conceder por ninguna vía**. La pantalla de Usuarios ofrece dos roles (`tenant_admin`, `security_guard`), el tipo está escrito a mano como unión de esos dos, y el servidor lo blinda además en `assertOperationalRole`. **Cero personas con el rol en producción, sobre 41.** Consecuencia: `FLOW-007` desplegó la firma del informe por el consejo (`identidadParaFirmar`, `index.ts:4898`, con su comprobación propia de membresía activa) y **es inejecutable — capacidad muerta en producción**. Tres entregas: conceder el rol · las pantallas de lo que YA tiene permiso · paz y salvo sujeto al abogado. `G0`–`G4` superadas; **`G5` es quién mantiene el consejo al día cuando la asamblea lo renueva** | 🔴 **`RN-01` es el hallazgo que ordena el diseño:** `tenantUsers` tiene id `{tenantId}_{uid}` — **una membresía por persona y conjunto, con un único campo `role`**—, así que poner `role: "committee"` le **quita al consejero su condición de residente** y con ella su unidad, su estado de cuenta y su firma del reglamento (`residentOwnUnit` exige `role == 'resident'`). Un consejero es un propietario. Por eso la marca es **atributo** (`isCommittee`) y no valor de `role`. Segundo hallazgo: **los permisos van por delante de las pantallas** — ya lee `documents`, `monthlyReports` no-borrador y `clearanceCertificates`, y solo tiene **una** entrada de navegación (`navigation.ts:125`). Y el corolario del plural: sustituir `tenantRole(...,'committee')` son **tres** sitios en `firestore.rules` (1100, 1441, 1449) más el servidor |
| [PRD-V-PLAT-006 — Buzones reales en conjuntos sin cliente: la puerta](funcionales/PRD-V-PLAT-006-buzones-reales-en-conjuntos-sin-cliente.md) | **Discovery — BLOQUEADA por tres decisiones de David** (2 sep 2026). Es la «fase 2» de `DATO-001`. **Se midió antes de escribirla y el criterio del chip resultó imposible por construcción**: `isExample` lo llevan los NUEVE conjuntos de producción, **incluidos los dos del trial**, cuyo administrador se registra con su correo real por diseño — una puerta «`isExample` ⇒ solo buzones inertes» rechazaría a cualquier prospecto. Y `DATO-001` limpió siete direcciones por su forma y **dejó once** en `people` (dos conjuntos demo, mayo–junio, ninguna con correo recibido). **Nada construido a propósito**, salvo el barrido en seco que lo midió (`functions/scripts/informe-correos-en-conjuntos-de-ejemplo.mjs`). Decisiones: **D1** qué marca a un conjunto sin cliente (recomendado: campo explícito, nunca derivado) · **D2** qué se admite (dominios inertes + lista del equipo) · **D3** entrada, salida o ambas (recomendado: **la salida primero**, que protege también las once) | Tanda 6. Sale del chip «cerrar la puerta del alta» de `docs/pendientes.md`. **El equipo valida con buzones reales dentro de conjuntos de ejemplo** (`david.macar.18+…@hotmail.com`, el iPhone de `PLAT-005`), así que sin lista del equipo la puerta rechaza la validación del producto. Los dominios inventados por las semillas (`santamaria.co`, `elnogal.co`, `privadapalmas.mx`…, 28 cuentas de Auth) quedan fuera a propósito: son las cuentas de entrada de las demos, y `TBD` de quién son esos dominios |
| [PRD-V-FEAT-004 — Estado de cuenta de la unidad y certificado de paz y salvo](funcionales/PRD-V-FEAT-004-estado-de-cuenta-y-paz-y-salvo.md) | **EN PRODUCCIÓN Y ENCENDIDA** (26 ago 2026, tarde). Esta celda decía «MVP construido y en staging, pendiente de validar y NO en producción». Estado de cuenta con PDF, certificado con emisión y anulación, portal del residente, cartera del administrador y lote. **Tres cosas de la ficha se corrigieron al construir**: §11.3 pedía un índice por `dueDate` que **habría roto R2 en silencio** —falta en el 27% de los cargos y un `orderBy` los descarta—; §11.1 y §11.3 **se contradecían** sobre el lote —callable con Storage contra «no hay segunda forma de hacer PDF»— y ganó el navegador con un archivo; y **R6 no se implementa**, porque acreditar una fecha pasada exige saber qué se debía ese día y los pagos no tienen fecha. **Ya NO está condicionada por `FIX-002`** (26 ago 2026): el dato está migrado y el parche de las tres vías se retiró — mirar la etiqueta pasó de necesario a **peligroso**, porque dos unidades homónimas se bloquearían entre sí. **CERRADA el 26 de agosto (tarde)**: front desplegado (`rollout-2026-08-26-002`) y `producto-estado-de-cuenta` **ENCENDIDA** en global, sin kill switch. **Validada por el RECHAZO, no por la emisión**: pedir el paz y salvo de una unidad que debe devuelve «saldo pendiente de 4.160.000, desde 2026-03 (3 períodos)» — lo que prueba de una vez que la bandera corre, que la guarda funciona y que **no crea nada** (`clearanceCertificates` sigue en 0, medido después). El servidor la comprueba en `emitClearanceCertificate`; **`cancelClearanceCertificate` NO, a propósito**, para que apagarla no deje papeles sin forma de retirarlos. **Y un defecto propio, cazado mirando la pantalla:** el estado de cuenta ordenaba por `dueDate` y en `T2-503` los períodos salían `05·03·04·06` con el saldo acumulado al lado. Ahora ordena por `period`. **Las once pruebas del fichero no podían verlo**: todas usaban cargos que vencen en su propio mes, y con esa entrada los dos órdenes dan lo mismo **30 ago:** el documento se nombra por país — constancia de no adeudo (MX), certificado de expensas (EC) — vía `vocabulario-pais`, en producción |
...
| [PRD-V-FIX-003 — Una sola definición por métrica del Panel de Control](funcionales/PRD-V-FIX-003-una-definicion-por-metrica-del-panel.md) | **EN PRODUCCIÓN y VALIDADA CON OJOS** (30 ago 2026, `d692be3`). Iniciativa `UX-004`, continúa `UX-003`. Construida el mismo día que se escribió. **`CA3` es el único criterio no visto en pantalla** —exige un conjunto con cobros emitidos y nada saldado, y la sesión del navegador solo alcanza Santa María—: está cubierto por prueba y por falsación, que no es lo mismo | **El defecto está medido, no supuesto.** El «% recaudo» del Panel de Control mide **un mes** y el de Cartera **hasta doce períodos**, con el mismo rótulo y sin declarar ninguno cuál: **divergen en los siete conjuntos de producción** —Palmas y Nogal dicen `0,0%` contra `50,0%`; Las Playas `100,0%` contra `76,6%`—. **Y en cuatro de los siete el panel afirma hoy en rojo un recaudo del 0,0% donde no hay ni un cobro emitido ese mes**: «sin datos» disfrazado de «lo peor», que es justo lo que la función vecina del mismo fichero ya evita y explica en un comentario. **Lo que NO es, y diagnosticarlo mal habría costado caro:** no hay aritmética duplicada — `buildBillingTrend` y `computeCollectionSummary` usan las mismas funciones puras y su guardián las vigila. Es la ventana y el rótulo. Incluye la tercera copia del rótulo de tipo de ticket (`other` = «Otros» en el widget, «General» en el módulo), dejada fuera a propósito en `UX-003`. **Sin datos, sin reglas, sin functions y sin bandera: reversible con un rollout** |

| [PRD-V-FLOW-006 — Interés de mora legal y convenio de pago (Ecuador)](funcionales/PRD-V-FLOW-006-mora-legal-y-convenio-de-pago.md) | **Discovery** (3 sep 2026) — `G0`, `G2`, `G4` y `G6` superadas; **`G1` NO** (cero clientes: no hay cartera vencida real que medir) y **`G3` y `G5` en AMARILLO**: cinco puntos esperan abogado ecuatoriano, y **nadie tiene asignado registrar la tasa del BCE cada mes** — sin tasa no se devenga. 19 criterios, **siete deben fallar** | Candidato **`B5`** de Habitanto fusionado con el convenio, porque **la ley los encadena**. Sale del **Decreto 462** (31 jul 2026, posterior al corte de conocimiento: la investigación se hizo con búsqueda web). **Riesgo ALTO: toca `aplicarPago`, en producción.** Medir antes corrigió dos supuestos: **`interes_mora` YA existe** como concepto con cuenta `1.4` —la ficha no lo crea— y **`agreement` en el código NO es un convenio**, son los acuerdos del consejo (341 falsos positivos si se busca por ese nombre). Y destapó que **`aplicarPago` ordena solo por fecha**, sin mirar el concepto: no cumple la cascada de cinco niveles que el decreto fija. Decisiones de David: **por país del conjunto** · MVP = **interés + cascada + convenio** · lo no nombrado por la ley cae en «ordinaria» · **escribir con supuestos declarados** |
| [PRD-V-FLOW-007 — Informe económico mensual anclado al banco, emitible y publicado](funcionales/PRD-V-FLOW-007-informe-mensual-anclado-al-banco.md) | 🟢 **ENTREGAS 1 Y 2 EN PRODUCCIÓN, Y LA BANDERA ENCENDIDA EN LOS NUEVE** (entrega 1 el 3 sep, `7b87e2b`; entrega 2 el 4 sep, `6e5524e`; global el 4 sep) — cálculo compartido con espejo byte a byte, saldo inicial real, las dos partidas nuevas, el aviso falso de «Fondo insuficiente» cerrado, y el informe emitible con firmas. **Solo queda la ENTREGA 3** (publicar al residente), **bloqueada por las cinco preguntas al abogado ecuatoriano**, que son de David. *(Esta celda decía «entregas 2 y 3 en Discovery» hasta el 6 de septiembre.)* 🔴 **Y esta ficha dejó CAPACIDAD MUERTA en producción:** `identidadParaFirmar` admite que **el consejo** firme el informe —con comprobación propia de membresía activa— y **el rol `committee` no se puede conceder por ninguna vía**, así que no la ejerce nadie. Lo destapó `PRD-V-PLAT-004`. Al construirla, **`R5` quedó FALSIFICADO** (la deuda a proveedores vive en los egresos, no en `vendors`) y **`CA3` fallaba de verdad** con centavos. Puertas: `G0`, `G2`, `G4`, `G5` y `G6` superadas — `G0`, `G2`, `G4`, `G5` y `G6` superadas; **`G1` NO** (cero clientes) y **`G3` en AMARILLO**: el alcance de la publicación al residente espera la misma respuesta legal que `FLOW-006`. 18 criterios, **siete deben fallar**, uno es la falsación. **`G5` SÍ se supera y es la diferencia con `FLOW-006`**: el dueño ya existe, es el administrador que hoy emite el informe a mano | §3.7 de la sesión con la administradora, **ascendido por la investigación legal**: publicar el informe mensual es obligación del Decreto 462 **cuya sanción es la remoción del administrador**. Medir corrigió el tamaño de la ficha en las dos direcciones: **cuatro de las seis partes de la columna vertebral YA se calculan** —`buildFinancialStatement` ordena por el código del plan y `fundBalance` es el saldo final, solo que no se llama `closingBalance`—, pero **el informe archivado es una reimplementación que ya se desvió DOS veces** (`R12` y `R16`, registradas en `payments.ts:267` y `:324`). Tres hallazgos más: **los tres consumidores pasan `openingBalance = 0`** aunque el dato exista, y de ahí sale un aviso falso de «Fondo insuficiente» **vivo hoy**; **la publicación al residente está DENEGADA por una regla desplegada**, no pendiente; y **`firma`/`signature` existen y son otra cosa** — la aceptación del residente de un acuerdo, no la firma de un informe |
| [PRD-V-FLOW-008 — Cuentas por pagar con calendario de cuotas](funcionales/PRD-V-FLOW-008-cuentas-por-pagar-con-calendario-de-cuotas.md) | 🟢 **LAS TRES ENTREGAS EN PRODUCCIÓN** (4 sep 2026, `3f0de0a`), bandera **ENCENDIDA EN LOS NUEVE** desde el 4 de septiembre de 2026, por el valor global (esta celda decía «apagada en los nueve»); ⚠️ **las overrides de canario de `tenant-santa-maria` y `conjunto-las-playas` siguen puestas**, así que para apagar los nueve hay que usar el **kill switch** y no `enabled: false` y **`R8` cerrado** — `G0`, `G2`, `G4`, `G5` y `G6` superadas; **`G1` NO se supera y David aceptó su ausencia explícitamente** (cero clientes), y **`G3` en AMARILLO a propósito**: toca el núcleo del estado financiero y **una cifra que YA está en producción**, y endurece una regla, lo que **invierte el orden de despliegue** (functions → front → reglas). 14 criterios, **seis deben fallar**, uno es la falsación. **Las dos preguntas abiertas las cerró David el mismo día, las dos en «no en el MVP»**: sin saldo por cuota y sin recordatorios — el MVP se queda con **el calendario y nada más** | **§3.2 de la sesión con la administradora**, y NO de los candidatos: ninguno de los diez de la categoría `E` lo cubre — el hueco salió de **escuchar**, no de navegar. «La del seguro, yo pago en **once cuotas**». Medir corrigió el tamaño en las dos direcciones: no falta solo el calendario, **tampoco existe el pago parcial de un egreso** y `ledgerEntryId` es **singular**, así que once cuotas no caben. Y destapó lo que más importa: **esta ficha ROMPE la deuda a proveedores de `FLOW-007`** —que suma el importe COMPLETO de todo egreso en `registrado`— y por eso la corrección va dentro. Baseline: **32 de 52 egresos sin vencimiento**, y **el caso que la motiva ya está en la base aplanado** («Póliza de seguro del inmueble (trimestral)», un solo pago) |
| [PRD-V-FLOW-005 — Autorizar la visita que llega sin avisar](funcionales/PRD-V-FLOW-005-autorizar-la-visita-que-llega-sin-avisar.md) | **EN PRODUCCIÓN** (30 ago 2026, `a8e2243`), bandera `producto-visita-no-anunciada` **solo en `tenant-santa-maria`** — el único conjunto con push, sin el cual la vía A nace inservible. Iniciativa `PH-003`. **Sin validar en pantalla todavía:** el portal de portería pide rol de guardia y el de residente rol de residente; `CA4` (la carrera entre dos residentes) y el push de la petición **piden dos dispositivos y dos personas**, y no hay suite que los sustituya | Hoy **toda visita nace de la mano del residente**, por QR emitido de antemano; la que llega sin avisar no tiene camino. Dos vías: el residente **autoriza desde su teléfono** (espera 5 min), o el guardia **llama por fuera y autoriza declarando el medio** (sin espera). **Lo que hace útil el registro no es que la visita quede anotada, sino quién autorizó y por qué medio.** La variante `registro_simple` ya trae media vía B, pero **no autoriza: consuma** —crea el pase ya dentro—. Rompe la exclusividad entre `qr_full` y `registro_simple`, porque la visita repentina ocurre en los dos. Estado nuevo *pendiente de autorización*, y **la expiración se deriva del sello de tiempo, no de un cron**. Abarata el frente que haya **cero visitas de portería en la historia de los dos ambientes**: se estrena un flujo, no se migra. **Depende del push (`PLAT-005`)**: sin él la petición cae en una campana que nadie mira |
| [PRD-V-FEAT-005 — Un padrón sin duplicados](funcionales/PRD-V-FEAT-005-un-padron-sin-duplicados.md) | **EN PRODUCCIÓN** (30 ago 2026, `944d42c`), bandera `producto-padron-sin-duplicados` **solo en `tenant-santa-maria`**. Iniciativa `ONB-002` (salió de `AI-ONB-001`), **sin IA**. **La primera fusión se ejecutó en producción**: siete «David Carmona» en uno, el padrón de 24 a 18 personas, cero referencias huérfanas. Sin ver queda `CA8` (el estado vacío), que exige un conjunto sin duplicados con la bandera encendida | **13 de 68 personas de producción estaban duplicadas** —la ficha decía 11 y **eran la regla de nombre a solas**; documento y correo añaden un racimo que esa regla no ve—, y 21 de 68 no tienen documento. **No entraron por el importador**, que sí deduplica: entraron por otros caminos, y ninguna puerta mira hacia atrás. **No lleva IA y decirlo es parte del trabajo**: las tres reglas determinísticas los encuentran; la IA tiene sitio en la cola larga, y esta ficha es lo que permite **medir si aporta algo**. **El riesgo vivía en la fusión, y construir corrigió la ficha en cuatro puntos:** el inventario de referencias estaba mal en las CUATRO filas —derivarlo **de los datos** encontró `packages.deliveredToId` y `packages.receivedBy`, siete cada uno, que ningún nombre delata, y descartó `tickets.residentId`, que lleva un uid: por nombres habría repuntado **29 de 43**—; «David Cancelo» y «Luis Otero» **comparten documento**, así que agrupar por cierre transitivo habría propuesto fusionar dos personas distintas; y la regla de las cuentas habría bloqueado el caso real, porque dos fichas apuntan al **mismo** uid. **Y lo que sostiene el invariante no es la lista: es el barrido** que aborta antes de escribir. Sin `snapshot` no hay fusión |
| [PRD-V-FEAT-006 — Unir varias columnas del archivo en un campo de la persona](funcionales/PRD-V-FEAT-006-unir-columnas-en-un-campo.md) | **Lista para PRD · v1.1** (1 sep 2026) — `G0`–`G3` superadas, **`G1` NO**: `importRuns` está vacío en producción, así que no hay tráfico contra el que leer su métrica. Diecisiete criterios, **doce con fixture que ya existe** | Sale de `AI-ONB-001` y **no lleva IA**. Hoy el mapeo es una columna → un campo, así que un padrón con «Nombres»+«Apellidos» o «Torre»+«Apto» **no tiene entrada ni a mano**. **La v1.0 duró un día: se le construyeron fixtures el mismo día y le encontraron SIETE huecos, cuatro verificados contra el código** — entre ellos que `RN-U4` se defendía con una razón falsa (`propietario residente` **sí** está en `ROLE_ALIASES`) y que la consecuencia «se verá Unidad no encontrada» es **falsa importando unidades**, porque `bulkCreateUnits` crea sin buscar nada. **Por eso la v1.1 se acota a `person`**: ahí el valor unido es una REFERENCIA y un error se ve; en una unidad es la IDENTIDAD y sembraría un padrón paralelo en silencio. Las fixtures de `unit` se conservan como línea base de Fase 2. **Enmienda el `RN-02` de `FEAT-002`** |
| [PRD-V-FEAT-007 — Modo oscuro elegible por el usuario](funcionales/PRD-V-FEAT-007-modo-oscuro-elegible.md) | **EN PRODUCCIÓN y ENCENDIDA EN LOS NUEVE CONJUNTOS** (3 sep 2026), por el valor global `featureFlags/producto-modo-oscuro` `enabled: true` y **sin overrides** — verificado calculando el valor **efectivo** conjunto por conjunto con la misma precedencia de `resolve.ts`: **9 de 9, los nueve por `valor_global`**. **Esta celda dijo «canario vivo, 1 de 9» hasta el 3 de septiembre por la tarde**, cuando ya llevaba horas en los nueve: se corrigió al medir para el reporte de estado, no al desplegar. **La override del canario se RETIRÓ al subir**, y no era limpieza: con ella puesta, un `enabled: false` futuro habría apagado ocho y dejado uno encendido. Las TRES entregas construidas y el ciclo **encender → apagar → encender** hecho: al reencender, **el residente recuperó solo su elección**. **NO marcada Productiva**: quedan cuatro criterios —las 7 pruebas de reglas **escritas y SIN CORRER, y ahora se sabe que SÍ se pueden correr**: esta celda decía «no hay Java en el equipo» y era falsa —el JDK está en `~/.local/jdk`, documentado en `CLAUDE.md`—, así que ya no hay excusa para dejarlas sin ejercitar, el destello (`CA5`/`CA6`), la impresión con ojos (`CA11`/`CA15`/`CA19`) y `CA7`—. `G1` sigue **aceptada vacía**, no superada | Épica `UX-006`. Las cuatro decisiones las cerró David; **resuelve además la decisión abierta de `UX-005`**. **Aparecieron SEIS formas de color literal, de una en una**, y la quinta —un hexadecimal dentro de un degradado— era el **lienzo que comparten los tres portales**: la vio David en una captura y ninguna prueba la veía. Un juicio mío fue falso por el camino: dije que los hexadecimales en atributos de JS «casi todos sobreviven», y era cierto para las gráficas y **falso para los mapas de superficie**. **No se tematizan a propósito** la marca del cliente, el QR y la paleta del avatar, y el guardián los protege **al revés**: enrojece si se quedan sin color |
...
| **En producción, no productiva** | **1** | **`FLOW-004`** — desplegada, con datos y con todos sus criterios cumplidos; `G5` abierta porque no hay a quién |
...
| Qué | Por qué no está |
|---|---|
| **`PRD-V-PLAT-004` — Alcance del rol Consejo** | ✅ **ESCRITA el 4 sep 2026** — ver la fila de arriba. Este candidato decía «ocho PRD le dan capacidades al consejo que hoy no puede alcanzar» y se quedaba corto: al medirlo, **el rol no se puede conceder por ninguna vía y no lo tiene nadie** (0 de 41). `canAccessPath` no solo lo deja en `/admin/documents`: con `role: "committee"` lo deja **fuera de `/resident` entero** |
...
| **`PRD-VAI-FEAT-001` — Leer el archivo que trae el cliente** | Es una de las cuatro de Drive, y su **alcance cambió el 30 de agosto**: incluye PDF y fotos por decisión de David, contra lo que argumentaba `docs/hoja-de-ruta-ia.md`. Al migrarla hay que reescribir ese alcance, no copiarlo. **No se escribe todavía porque hoy no pasaría sus propias puertas:** sin corpus (15–25 archivos reales, y hay cero guardados) no hay con qué evaluar ni con qué estimar coste. **La parte de duplicados ya salió de aquí** y es `PRD-V-FEAT-005`, funcional y sin modelo |
| **`UX-005` — Tableros del panel a la medida del administrador** | Encuadrada el 30 de agosto. **No se escribe todavía por decisión de prioridad de David**, y porque va detrás de `UX-004`, que aún no está construido. Le falta además una decisión de modelo: si la preferencia es por usuario o por conjunto |
