...
| [PRD-V-PLAT-005 — Notificaciones push al residente](funcionales/PRD-V-PLAT-005-notificaciones-push-al-residente.md) | **Borrador 0.1** (29 ago 2026). Tercer canal del sistema de notificaciones: el aviso que ya se escribe en `notifications` llega además al hub del teléfono, vía FCM/Web Push, sin app en las tiendas. Bandera `producto-notificaciones-push`, nace apagada. **0.2:** D1 y D2 cerradas —tope de 5 dispositivos, iconos generados del SVG (solo emblema, 192/512/180)—; queda solo la dependencia de consola (par VAPID, la hace David) | Medido antes de escribir: `registerWebPush` existe con **cero consumidores**, sin SW ni manifest ni lado emisor; `notifications` tiene 507 docs en producción y `pushTokens` 0. El envío se engancha en el embudo único `createNotifications` (gemelo bueno: el correo en `deliverResidentNotifications`). En iOS exige añadir a pantalla de inicio — el riesgo es de adopción, no de ingeniería. `G5` se abre con el primer dispositivo real recibiendo |
