...
| [PRD-V-FLOW-001 — Prorrateo de un gasto entre las unidades](funcionales/PRD-V-FLOW-001-prorrateo-de-un-gasto-entre-unidades.md) | **MVP COMPLETO Y EN STAGING**, validado por navegador contra la base el 25 de agosto de 2026 (`728451f`…`abcbaad`): reparto por coeficiente, vista previa, trazabilidad en los dos sentidos y anulación de la corrida con motivo. **Su servidor SÍ está desplegado en producción desde el 26 de agosto, pero APAGADO** —la bandera `producto-prorrateo-de-gastos` no existe como documento y el front sigue en `develop`—. Y aunque se encendiera **no podría correr**, por datos: 0 de 88 unidades con coeficiente y 74 de 87 sin propietario, así que R2 y R5 bloquean antes de calcular. **La mitad del trabajo la hacía ya `PLAT-001`** —`repartirPorCoeficiente` y cuatro de los seis campos «nuevos», que `domain.ts` ni declaraba—. Dos defectos propios cazados: el reintento idempotente chocaba con la guarda de repetido, y el aviso de doble cobro estaba apagado en **48 de 130 egresos** por categorías fuera del tipo | Tanda 2. Une egreso y cartera, que hoy **no se hablan**: no existe puente `Expense`→`BillingStatement`. Incluye **anular una corrida entera**, que hoy tampoco existe. **Bloqueada por `PRD-V-PLAT-001`**. Cerradas: concepto elegido de los siete existentes con `extraordinaria` por defecto (**sin ampliar `BillingConcept`**) · reparto a subconjunto fuera del MVP |
...
| **Servidor en producción pero APAGADO** | 2 | `FLOW-001`, `FEAT-004` — desplegados el 26 de agosto con sus reglas e índices. **La pregunta ya no es cuándo se suben, es cuándo se encienden** |
| Lista para desarrollo | 1 | `FLOW-003` |

**Las dos que están desplegadas y apagadas, y por qué**, porque es la pregunta que se va a repetir:

- **`FLOW-001`** — aunque se encendiera **no podría correr**: 0 de 88 unidades con coeficiente y
  74 de 87 sin propietario, así que R2 y R5 bloquean antes de calcular. **Encenderlo sin sembrar
  sus datos sería la enésima capacidad viva sobre una tabla vacía.**
- **`FEAT-004`** — **ya no depende de `FIX-002`**, que se cerró el 26 de agosto. Lo único que falta
  es encender la bandera y subir el front.

**Y el matiz que hay que decir entero:** su código corre en producción desde el 26, pero sus
banderas **no existen como documento** y el catálogo las tiene en `false`, así que allí no hacen
nada. El front sigue en `develop` — `master` va 68 commits por detrás. **Desplegar y encender son
dos actos**, y solo se ha hecho el primero.
