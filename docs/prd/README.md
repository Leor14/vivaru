...
| [PRD-V-FEAT-005 — Un padrón sin duplicados](funcionales/PRD-V-FEAT-005-un-padron-sin-duplicados.md) | **Lista para desarrollo** (30 ago 2026). Iniciativa `ONB-002` (salió de `AI-ONB-001`), **sin IA** | **11 de 68 personas de producción están duplicadas** —«David Carmona» ×7, en dos unidades, con dos documentos y el mismo correo—, y 21 de 68 no tienen documento, que es uno de los campos con los que se deduplica. **No entraron por el importador**, que sí deduplica: entraron por otros caminos, y ninguna puerta mira hacia atrás. **No lleva IA y decirlo es parte del trabajo**: la detección de los once se hizo con una regla determinística de treinta líneas; la IA tiene sitio en la cola larga, y esta ficha es lo que permitirá **medir si aporta algo**. El riesgo vive en la fusión: **`mergeUnits` prometía «TODAS las referencias» con una lista de nueve escrita a mano y eran dieciocho** —eso explica los huérfanos de Santa María—, así que el inventario **se deriva y tiene guardián**. Y volvió a pasar al escribir la ficha: la primera medición dijo que ninguno de los siete estaba referenciado, y **dos lo están desde arrays de unidad**. Sin `snapshot` no hay fusión |
