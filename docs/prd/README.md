...
| [PRD-V-FEAT-007 — Modo oscuro elegible por el usuario](funcionales/PRD-V-FEAT-007-modo-oscuro-elegible.md) | **Lista para PRD** (3 sep 2026) — `G0` y `G2`–`G6` superadas. **`G1` NO se supera y David aceptó su ausencia el mismo día**: producción tiene cero clientes, la adopción no se puede medir, y `CA12`–`CA14` la sustituyen con métrica de **terreno**, que no es lo mismo. Se construye igual porque el valor es de accesibilidad y no de conversión. Diecinueve criterios, **cinco de ellos deben fallar** | Épica `UX-006`. **Las cuatro decisiones abiertas las cerró David**: por usuario · residente y admin · claro u oscuro **ignorando el sistema operativo** · el informe del consejo siempre en claro. **Cierra también la decisión abierta de `UX-005`**. Medir antes cambió la ficha en cuatro sitios: la palanca del tema son **~25 tokens semánticos y no los 127** —las otras 64 declaraciones son escalas con nombre de color, que se re-mapean y no se invierten—; hay un tercer bloque que nadie contó, **35 reglas de color literal en el propio `globals.css`**; **los PDF no «no deben» heredar el tema: no pueden**, porque se dibujan desde datos con `jspdf` y `pdfkit`, y el único que hereda el DOM es el informe del consejo por `window.print()`; y **Vivaru ya responde al modo oscuro del sistema en producción sin que nadie lo decidiera** —21 reglas de `prefers-color-scheme` verificadas en `grupovivaru.com`, no deducidas—, lo que convierte «ignorar el sistema» en un cambio visible sobre una página fuera de alcance |
