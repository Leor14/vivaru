...
| **Bloques de Habitanto** | 🟢 **LO CONSTRUIBLE, HECHO** (10 sep) — `FLOW-007` entregas 1 y 2 y `FLOW-008` encendidas en los nueve; **`FEAT-008`** (§3.5, la foto del medidor, tres entregas) y **`FEAT-009`** (§3.8, presupuesto contra ejecución, dos entregas) en producción **con la bandera apagada** | 🟡 **Verificar §3.10** —el pase recurrente ya existe— · **encender** `FEAT-008` o `FEAT-009` donde haya quien mida consumos o cargue un presupuesto: es decisión, no código | 🔴 **`FLOW-006` y la entrega 3 de `FLOW-007` BLOQUEADAS por el abogado** · 🟠 **`FIX-001` entrega 2** (reserva por área): van 10 de 11 · ◇ Fase 2 de `FEAT-009` (reformado, consejo en la app, línea en el informe) | ◇ Quedan **37 P1, 42 P2 y 12 P3** de los 108 candidatos |
