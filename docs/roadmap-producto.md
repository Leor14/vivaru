...
**Las puntuaciones son de la v0.4**, recalculadas el 15 sep: cada estimación por el nivel de su
prueba, y el Ingreso a 2 en lo que ella pidió. **Los grupos los fijan el freno y la puerta de lo
obligatorio, no la puntuación**; la puntuación ordena dentro de cada grupo.
...
|---|---|---|---|---|---|
| **L-27** | El presupuesto privado ya existe: **encenderlo en Santa María** | 8,3 | Relleno | 0,45 | Permiso de bandera |
...
| **L-22** | Subir el informe del contador, con la categoría forzada a `financiero` | 3,65 | Relleno | 0,85 | — |
| **L-21d** | Defecto: PQRS enseña la unidad como un id crudo y al residente como «Residente» | 3,6 | Ganancia rápida | 1,1 | — |
| **L-23** | Correo de cobro solo a los deudores (va después de D-1) | 3,1 | Ganancia rápida | 1,4 | — |
| **L-24** | Tendencia de cartera mensual y acumulada del año | 3,1 | Relleno | 0,9 | — |
| **L-07** | Ver las unidades de un mismo dueño | 2,85 | Relleno | 1,0 | — |
| **L-20** | Paquetería: qué empresa lo trajo, en qué estado llegó y la entrega a la vista | 2,6 | Relleno | 1,2 | — |
| **L-29** | Quién registró cada entrada y salida: hay 309 ingresos sin autor en producción | 2,3 | Ganancia rápida | 1,8 | — |
| **L-21** | PQRS: evidencia de la solución y Excel del detalle | 2,1 | Relleno | 1,8 | — |
| **L-13** | Cuántas personas vieron un comunicado | 1,6 | Relleno | 1,5 | — |

### 3. Vale la pena, pero espera una decisión de David (freno B)

| Id | Qué | Qué hay que decidir | Puntuación |
|---|---|---|---|
| **L-04** | Santa María no tiene país: dice «porcentaje de copropiedad» en vez de «coeficiente» (el punto 4 del menú) | Poner `country: CO`, con permiso | 6,1 |
| **L-17** | Encuestas: un nombre más cercano; el editor de formularios ya existe | El nombre (con él, pasa a XS) | 4,4 |
| **L-08b** | El residente crea su propio visitante frecuente | Si puede hacerlo él solo | 3,9, Ganancia rápida |
| **L-18** | El reglamento pide firmas que en propiedad horizontal nadie da; el modo sin firmas ya existe | El valor por defecto por país | 2,95 |
| **L-03** | El alta de unidad pide área y valor del seguro, que nadie lee | ¿Retirar, esconder o por país? ¿Entra la H.39? | 2,9 |
| **L-06** | La inmobiliaria como tercera figura (candidato `A3`) | ¿Recibe cobros, avisos o acceso? | 2,85 |
| **L-32** | Números de emergencia y preguntas frecuentes | ¿Del conjunto o de la plataforma? ¿Quién redacta? | 2,8 |
| **L-19** | Reservas con valor, depósito, aforo en personas y protocolo | ¿El valor solo se informa o genera un cargo? | 2,4, Ganancia rápida |
| **L-10** | Personal de servicios con horario (va después de L-08a) | ¿Catálogo del conjunto o por unidad? | 2,1, Ganancia rápida |
| L-15 · L-09 · L-14 · L-16 | Clase del comunicado · foto del visitante · registro de comunicados borrados · clasificados de la copropiedad | La taxonomía · cómo se ve y se guarda la foto · qué se conserva · quién publica | 2,6 · 2,6 · 2,1 · 2,0 |

### 4. No vale la pena ahora, o está frenado por terceros

- **Las seis de menor puntuación, todas de 1,3 o menos:**
  - L-26, matriz de mantenimientos (1,3)
  - L-30, minuta de vigilancia (1,1)
  - L-25, consumo de las zonas comunes (1,05)
  - L-28, convivencia y descargos (1,0)
  - L-05, la exención por mora en el parqueadero (0,76, y depende de `A8`)
  - L-01b, un tema más llamativo (0,65)
...
| **Comunicaciones y comunidad** | D-2b, D-2 | L-13 (puntúa poco: 1,6) | L-17, L-18, L-15, L-16, L-14 | L-31 | L-12 |
...
- **Por qué.** Aplicar la decisión de David de que la administradora cuenta como prospecto destapó que la
  regla de confianza no era monótona: tener más pruebas podía bajar la nota. La v0.4 multiplica cada
  estimación por el nivel de su propia prueba, y entran las cuatro definiciones aprobadas.
- **Contra qué se verificó.**
  - El script (`docs/valoraciones/recalculo-lote-2026-09-15.py`) reproduce primero las 37 valoraciones
    de la v0.3 sin ninguna diferencia.
  - Con la v0.4, la tercera calibración conserva sus cuadrantes con el umbral de 4, así que **el umbral
    no se toca**.
- **Qué cambió en el apartado del lote.**
  - Pasa de 4 a 6 Ganancias rápidas: entran `L-23` y `L-10`.
  - `L-21d` baja de 5,5 a 4,0 y sigue siendo Ganancia rápida.
  - `L-27` pasa a ser la primera.
  - **Los grupos no cambian**: los fijan el freno y lo obligatorio.
...
- **Las banderas de medidor, presupuesto y tesorería, medidas de nuevo** resolviendo conjunto por
  conjunto: en producción, 2 de 10 —Las Playas y Lomas de Sayilbedra—, y no «solo Las Playas», como
  seguían diciendo la wiki, las fichas, el índice de PRD, el backlog y el tablero.
- **Los candidatos de Habitanto, recontados sobre las filas:** quedan **32 P1, 38 P2 y 12 P3** sin ficha.
  Circulaban cuatro cifras porque las filas no apuntaban qué tomó cada ficha; la regla y la lista de los
  tomados están ahora en `docs/prd/candidatos-prd-desde-habitanto.md`.
...
- **Tres overrides de bandera, retirados en producción** (con permiso de David): `producto-egresos-en-cuotas`
  en Las Playas y Santa María, y `producto-informe-mensual` en Santa María. Repetían el valor global, y con
  ellos puestos apagar la global no las apagaba ahí. `mover-bandera-de-conjunto.mjs` ganó la opción `quitar`
  para hacerlo sin un comando suelto; las dos banderas resuelven 10 de 10 por el valor global.
- **`develop`, llevado a `master`** (22:11 UTC), con un avance directo desde `2f035a3`. Producción sirve
  `build-2026-09-12-007` desde `a7ab743` a partir de las 22:18. El modo automático bloqueó el primer intento
  aunque David lo había pedido, y pasó al reconfirmarlo con el diff delante: en el front, un tipo y un
  comentario.
- **El «HOGARU» que no llegó a `develop` y el tipo de `distributionBasis`**, cerrados por la noche.
  «Superadmin Vivaru» en `ROLE_LABEL` —no se veía: no lo importa nadie— y «Usuario» a secas como relleno del
  nombre de la sesión, como había decidido `2e0be85`, que se quedó en una rama. El tipo de la base de reparto
  declara ya `consumption`, que el servidor escribe desde `FEAT-008`. Dos guardianes nuevos, cada uno
...
- **La política de reserva baja al área** (§17 de la ficha). Tres campos, y sin configurar todo sigue como
  hoy (`CA10`):
  - `blockOnDebt`: el área manda sobre el conjunto; `null` hereda del conjunto;
  - `minAdvanceMinutes`: de 0 a 10 080; 30 por defecto;
  - `autoApprove`: la reserva nace aprobada y el residente recibe «Reserva aprobada».

  El administrador lo configura en «Aprobación y morosos», y el residente lo ve en la ficha del área.
- **`CF9` no se cumplía, y se cerró en `amenities`**: el `update` solo miraba el `tenantId` nuevo, y un
  administrador podía quedarse el área de otro conjunto. El mismo patrón sigue en otros 19 bloques de las
  reglas: tarea aparte propuesta.
- **Verificada en vivo en staging**: la ficha, el aviso «Reserva aprobada.», la reserva `approved` con
  `autoApproved` y el aviso del disparador. David revisó el formulario.
- **Producción, en orden functions → front → reglas**: functions 14:10Z, front `build-2026-09-12-006`
  (`2f035a3`) y reglas `6f42ef47`, idénticas al repo. Radio medido antes: 0 de 35 áreas con los campos nuevos.
- **Antes de subir caducaron las dos credenciales** (CLI de Firebase y ADC). David las renovó, y no se
  desplegó nada sin poder verificarlo.
- Bancos: `npm test` **2066** · functions **1011** · reglas **494**; 13 mutaciones de falsación.
...
- **En producción** (`d91e1af`, solo front, sin bandera), con el mismo gemelo que egresos. **Dinero**: la
  fecha propuesta del cobro, la fecha contable al aprobar un recibo y al cruzar un anticipo, el asiento
  manual, el cargo de Cartera y su período, la campaña por coeficiente, los tres tableros, la tarjeta de
  vencimientos, y el estado y `lastPaymentAt` que escribe el cliente: **un cargo que vence hoy dejaba de
  estar pendiente por la tarde**. **Operativo**: comunicados, acuerdos, mudanza, encuestas, el panel, y
  desactivar un área, que **dejaba sin cancelar las reservas del mismo día**.
- **Medido antes de tocar el reporte de comité**: agrupa un momento por su día, y los asientos no pasan
  por ahí porque su `date` es texto en los dos ambientes (95 y 109 de 109). Si hubiera habido Timestamps a
  medianoche UTC, pasarlos a local los habría movido al día anterior.
- **Se quedan, con motivo**: la **portería** —sus visitas por invitación guardan el día UTC y casan hoy con
  la comparación UTC: dos errores que se compensan no se arreglan de uno en uno—, el **servidor** —su «hoy»
  solo decide «vencido», y arreglarlo pide una zona por conjunto que no existe— y su espejo del cliente.
- **Verificado**: guardián sobre todo `src/` con esas excepciones declaradas, falsado en nueve mutaciones,
  con el hueco dicho (fechas que salen de una variable). En producción, antes y después: `2026-09-11` a las
  23:12 y `2026-09-10` a las 23:19 en «Libro y fondos» y Cartera. `npm test` **1955**.
...
- **`PRD-V-FEAT-008` — la foto del medidor —, las tres entregas en los dos ambientes** (producción en
  `6fac4bc`), con `producto-medicion-de-consumos` **apagada en los nueve** y encendida solo en Palmas de
  staging. Sale de §3.5, el cuello de botella declarado de la administradora: la lectura con su foto
  —obligatoria para CERRAR el período, no para guardar—, el consumo calculado, el cobro como **tercera
  base de reparto** que ya existía, y el residente viendo su lectura y su foto, **y no la del vecino**:
  falsado sembrando un dato en la unidad de al lado.
- **`PRD-V-FEAT-009` — presupuesto contra ejecución —, las dos entregas en los dos ambientes**
  (producción en `7846bc1`), con `producto-presupuesto-anual` **apagada en los nueve** y ningún
  presupuesto guardado. Cargar por cuenta, comparar con lo ejecutado del año en curso y del cerrado,
  aprobar con la fecha del acta —y queda bloqueado **en las reglas**— e imprimir para la asamblea.
- 🔴 **Lo ejecutado NO se calcula en la ficha, y sumar asientos habría dado mal.** El núcleo **salta los
  ingresos por cuota** (`esRecaudoDeCartera`) y los recibe aparte, así que un ejecutado sacado del libro
  no tendría la mayor partida. Se lee del mismo hook que `/admin/reports`, con guardián — y en staging
  salió **idéntico a Reportes en las siete cuentas**.
- **«§3.8 encaja dentro del informe mensual» se corrigió al medir**: lo que no había que rehacer era el
  CÁLCULO, no la pantalla. El informe es mensual, congelado y con su bandera apagada en los nueve, y el
  consejo —que es quien lo lee— no puede leer el libro.
- ⚠️ **El año cerrado sale vacío hoy en los nueve conjuntos**: los 95 asientos de producción son de 2026.
  El uso legal real llega con la asamblea del primer trimestre de 2027.
- **§3.10 estaba cubierto y el reporte lo daba por pendiente.** El formulario de visitas del administrador
  tiene autorización de **larga duración** y categoría **Servicio**: el pase del proveedor recurrente ya
  existía. Comprobado en el código, no en pantalla; no se enlaza con el registro de proveedores.
- **Tres defectos de lectura, y los tres los encontró MIRAR:** el campo de lectura de `FEAT-008`
  conservaba el mes anterior al cambiar de período —una identidad de nodo, no un error de tipos—; y en
  `FEAT-009`, sin presupuesto, **cada egreso decía «Sobre-ejecución»** y el resultado presupuestado
  «equilibrio». Ninguna prueba miraba ese caso, que es como abre la pantalla cualquier conjunto.
- **Dos tropiezos de método, anotados para no repetirlos:** un `| tail` se tragó el fallo de un deploy de
  reglas y **el push salió igual** —la lección ya estaba escrita—; y un vigilante de despliegue que no
  podía acertar, porque llamaba al script sin sus argumentos y contaba el error como «esperando».
...
- **El informe del consejo TIRABA los pagos parciales** (`69ff374`). `/admin/reports` y
  `/admin/finanzas` daban «Saldo de fondos» distinto el mismo día —**−675.000 contra 725.000** en
  Santa María— porque el informe filtraba `status === "paid"` y **un cargo con pago parcial no es
  `paid`**. Medido antes de tocar: **10 cargos por $4.800.000 en TRES conjuntos**, no en uno.
  **Eran CUATRO sitios**, incluido uno que recalculaba con la fórmula rota un número ya calculado
  bien cinco líneas más arriba, y otro que construía a mano el mapa que `repartirRecaudo` devuelve.
  La fórmula única ya existía —`src/features/billing/collection.ts`— y **este fichero ya la
  importaba para «facturado»**: el «recaudado» se había quedado fuera de aquella unificación.
- **Lo sostenía un comentario que JUSTIFICABA la desviación**, describiendo el defecto con
  exactitud y llamándolo decisión deliberada. Llevaba ahí desde el 23 de junio.
- **La morosidad daba «106% de unidades · 19/18»** (`e8f3c37`), un porcentaje imposible en el
  documento que se le presenta al consejo. Santa María tiene 18 unidades activas y **19 con cargos
  vencidos**: la inactiva `T1-202` debe **$2.240.000**. **Se ensanchó el denominador y NO se filtró
  el numerador** —filtrar habría escondido esa deuda de «Mayores deudores» y contradicho el «Total
  vencido»—. También eran **dos** sitios: el cálculo y el rótulo que lo pinta.
- **El pie explicaba el denominador viejo** (`ac26068`), y **se introdujo al arreglar lo anterior**:
  la frase «sobre unidades activas» sobrevivió al cambio de fórmula.
- **Los tres los encontró MIRAR, no probar**, y los tres están verificados con ojos en producción
  con las predicciones escritas antes de mirar. `npm test` **1789**, functions **832**.
- **`PRD-V-PLAT-004` escrita** (alcance del rol Consejo), en Discovery y **bloqueada por `G5`**.
  Medirla cambió la ficha: **el rol `committee` no se puede conceder por ninguna vía y no lo tiene
  nadie (0 de 41)**, y `FLOW-007` dejó **capacidad muerta en producción** —la firma del informe por
  el consejo—. `RN-01`: la marca tiene que ser un **atributo**, no un valor de `role`, porque
  `canAccessPath` deja a un `committee` fuera del portal del residente entero.
- **`producto-informe-mensual` y `producto-egresos-en-cuotas`, ENCENDIDAS EN LOS NUEVE** (4 sep),
  por el global. Las overrides de canario siguen puestas: para apagar los nueve, **kill switch**.
...
- **Primer bloque de Habitanto convertido en ficha.** `PRD-V-FLOW-006` — interés de mora legal y
  convenio de pago, **en UNA sola PRD porque la ley los encadena**. Estado **Discovery**.
- **Sale del Decreto 462**, firmado el 31 de julio de 2026 y por tanto **posterior al corte de
  conocimiento del asistente**: toda la parte legal viene de `docs/investigacion-legal-ecuador-mora.md`,
  hecha con búsqueda web, no de memoria.
- **Medir antes corrigió dos supuestos y destapó un incumplimiento vivo:**
  1. **`interes_mora` YA existe** como `BillingConcept` con cuenta `1.4`, etiqueta y selector. La
     ficha no crea el concepto: crea **el cálculo y el orden**.
  2. **`agreement` en el código NO es un convenio de pago** — son los `CommitteeAgreement` del
     consejo. Buscar por ese nombre da **341 falsos positivos**.
  3. **`aplicarPago` ordena solo por fecha** (`ordenarPorAntiguedad`, `payments.ts`), sin mirar el
     concepto: **no cumple la cascada de cinco niveles** que el decreto fija. Hoy no es ilegalidad
     viva —no hay intereses ni gastos de cobranza en el modelo—, pero **el día que existan, reparte
     mal**. Por eso interés y orden van en la misma ficha.
- **Y dos gemelos que ya lo hacen bien:** `calcularSaldo` ya marca `overdue` cuando
  `vencimiento < hoy`, que es **exactamente** la regla legal de que la mora corre desde el día
  siguiente sin requerimiento; y `updateOverdueStatements` ya barre a diario, así que el devengo no
  necesita mecanismo nuevo.
- **Cuatro decisiones de David:** la cascada aplica **por país del conjunto** —México y Colombia
  intactos, y `CA2` lo exige comparando el reparto antes y después— · MVP = **interés + cascada +
  convenio**, dejando gastos de cobranza y fondo de reserva como **niveles declarados y vacíos** ·
  lo que la ley no nombra cae en «ordinaria» · **se escribe con supuestos declarados** y la tasa se
  guarda dentro del cargo para poder recalcular.
- **`G3` y `G5` quedan en amarillo a propósito**: cinco puntos esperan abogado ecuatoriano, y
  **nadie tiene asignado registrar la tasa del BCE cada mes** — sin tasa no se devenga nada.
...
- **Lo que falta de Habitanto, con nombre y tamaño:** el lote cerrado era **solo el bloque P0**. De
  los **108 candidatos** quedan **38 P1, 42 P2 y 12 P3** (`docs/prd/candidatos-prd-desde-habitanto.md`);
  las categorías con más P1 son **G·comunicaciones (6)**, **B·cartera y cobro (6)**, **E·egresos (5)**
...
- **`PRD-V-FEAT-007` — Modo oscuro elegible por el usuario**, en `docs/prd/funcionales/`. Épica
  `UX-006`. Estado **Discovery**: `G0` y `G2`–`G6` superadas, **`G1` no**.
- **Las cuatro decisiones abiertas, cerradas por David:** preferencia **por usuario** (`users/{uid}`,
  lo que **resuelve también la decisión abierta de `UX-005`**, que hereda el modelo) · alcance
  **residente y admin** · **claro u oscuro, ignorando el sistema operativo** · **el informe del
  consejo se imprime siempre en claro**.
- **Medir antes de escribir cambió la ficha en cuatro sitios**, y ninguno se deducía leyendo:
  1. **La palanca del tema son ~25 nombres, no 127.** `globals.css` declara 127 propiedades
     personalizadas, pero de las 68 de `:root` **solo cuatro son semánticas**; las otras 64 son
     escalas con **nombre de color** (`--slate-*`, `--brand-*`, `--danger-*`, `--amber-*` y 24 pares
     `--icon-*`). Un token que se llama `--slate-200` **no se puede invertir**: su nombre dice qué
     color es, no qué papel cumple. Las escalas **se re-mapean**; solo los semánticos se invierten.
  2. **Hay un tercer bloque que ninguna medición había contado.** Los componentes tienen cero
     hexadecimales, cierto — pero **35 reglas del propio `globals.css` pintan color literal**:
     `.soft-panel` con su degradado, los tooltips y seis sombras.
  3. **Los PDF no «no deben» heredar el tema: NO PUEDEN.** Paz y salvo, estado de cuenta y recibo se
     dibujan con `jspdf` **desde datos**, y el adjunto de correo con `pdfkit` en el servidor. De los
     tres `window.print()` que quedan, dos abren ventana nueva con estilos en línea. **El único que
     hereda el DOM es el informe del consejo** (`/admin/reports`), cuyo `@media print` **oculta pero
     no repinta** — y es justo el documento que se firma.
  4. **Vivaru ya responde al modo oscuro del sistema EN PRODUCCIÓN, y nadie lo decidió.** Al no
     existir `@custom-variant dark`, la variante `dark:` de Tailwind 4 es una media query pura.
     **Verificado en `grupovivaru.com`, no deducido:** el CSS servido tiene **21 reglas dentro de
     `@media (prefers-color-scheme: dark)`**, y con el sistema operativo en oscuro un botón del
     landing se pinta **ahora mismo** con un fondo pensado para oscuro sobre una página clara. Eso
     convierte «ignorar el sistema» en un **cambio visible sobre una página fuera de alcance**, y
     por eso la ficha lo lleva como criterio (`CA16`) con su valor de partida medido.
- **Dos avisos de `0.9.53` corregidos con evidencia:**
  **(a)** los 625 usos de color literal se miden en **1.048 fuera de marketing, en 145 ficheros**,
  con un patrón más amplio — el reparto importa más que el total: `components/features` 39 ·
  `shared` 26 · `(admin)` 18 · `(resident)` 13, y **`components/ui` solo 9**, así que la base del
  sistema de diseño está casi limpia. El peor fichero del repositorio es `GuardVisitors.tsx`, con
  **59 usos**, y está en un portal **fuera de alcance**.
  **(b)** **la trampa de Tailwind con los `.md` YA ESTÁ CERRADA**: `@source not "../../docs"` está
  en la línea 20 de `globals.css`, junto a las exclusiones de `tests`, `scripts` y `.github/prompts`.
  Escribir esta ficha **no puede** cambiar el bundle de producción.
- **`G1` cerrado decidiendo, no midiendo.** David **aceptó explícitamente** que la puerta de valor no
  se supere: producción tiene cero clientes y la adopción no se puede medir, y esperar por esa cifra
  sería esperar por un número que no cambiaría la decisión —el valor es de accesibilidad, no de
  conversión—. La ficha pasa a **Lista para PRD** el mismo día. **Queda escrito que aceptarla vacía no
  la vuelve superada:** el día que haya clientes, medir la adopción es deuda de esta ficha.
- **Sin código de producto.** Ni una línea: la ficha es la entrega.
...
- **Los cinco cabos sueltos, cerrados en un commit (`484e16a`), falsados por edición y EN PRODUCCIÓN** (`build-2026-09-02-001` ← `2584aa3`, al empujar `master` esa misma noche). Un
