 ...
 */
const OPERACIONES_CON_FEEDBACK = ["comunicaciones-redactar", "pqrs-asistir"];
async function runFeedback(request) {
    ...
    const clave = request.data?.operationKey;
    const conocida = OPERACIONES_CON_FEEDBACK.includes(clave);
    const operacion = conocida ? (0, catalog_1.findOperation)(clave) : null;
    if (!operacion) {
