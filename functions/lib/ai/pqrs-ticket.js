...
exports.ticketPerteneceAlConjunto = ticketPerteneceAlConjunto;
exports.variantePqrsDe = variantePqrsDe;
exports.construirEntradaPqrs = construirEntradaPqrs;
/** Default = comportamiento actual, igual que en `src/lib/config/module-variants.ts`. */
const VARIANTE_POR_DEFECTO = "con_sla";
 ...
 */
function ticketPerteneceAlConjunto(ticket, tenantId) {
    if (!ticket || typeof ticket !== "object")
        return false;
    const suyo = ticket.tenantId;
...
function construirEntradaPqrs(ticket, variante) {
    ...
    if (!mensajeCompleto)
        return { ok: false, reason: "ticket_sin_texto" };
    ...
    const entradas = historialCompleto
        .map((entrada) => texto(entrada?.message))
        .filter((t) => t.length > 0)
        .map((t) => ({
