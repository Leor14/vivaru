...
const visita_no_anunciada_1 = require("./visita-no-anunciada");
...
tenantId) {
    try {
        ...
        await (0, email_1.sendAccountEmail)({ to: email, fullName, link, variant, tenantId });
    }
    catch (error) {
...
async function sendOnboardingInvite(uid, email, fullName, tenantId, role, variant = "welcome") {
    try {
        ...
        await (0, email_1.sendAccountEmail)({ to: email, fullName, link: `/activar?token=${token}`, variant, tenantId });
    }
    catch (error) {
...
exports.createTenantOperationalUser = (0, https_1.onCall)({
    ...
    try {
        ...
        if ((0, trial_modules_1.esAmbienteDePrueba)(tenantSnap.data()?.status)) {
            const permitido = await (0, limites_de_intentos_1.consumirIntento)(`operativos-en-prueba:${targetTenantId}`, trial_modules_1.MAX_OPERATIVOS_EN_PRUEBA, trial_modules_1.VENTANA_DE_PRUEBA_MS);
            if (!permitido)
                throw new https_1.HttpsError("resource-exhausted", trial_modules_1.MENSAJE_TOPE_DE_PRUEBA);
        }
 ...
 */
exports.billConsumptionPeriod = (0, https_1.onCall)({ cors: http_config_1.callableCorsOrigins }, async (request) => {
    if (!request.auth?.uid)
        throw new https_1.HttpsError("unauthenticated", "Debes autenticarte.");
    const d = request.data;
    const tenantId = normalizeText(d?.tenantId);
    ...
    const actor = await assertActiveTenantAdmin(tenantId, request.auth.uid);
    await (0, feature_flags_1.assertFeatureEnabled)("producto-medicion-de-consumos", actor.tenantId);
    const r = await (0, medicion_de_consumos_1.generarCorridaDeConsumo)({
        tenantId: actor.tenantId,
        serviceId,
        period,
...
exports.registerWalkInVisit = (0, https_1.onCall)(async (request) => {
    ...
    const resultado = await (0, visita_no_anunciada_1.registrarVisitaNoAnunciada)(request.data, {
        uid,
    ...
    await deliverResidentNotifications(clave, request.data.tenantId, resultado.residentes, via === "app"
        ...
        : { visitante: request.data.visitorName, quienResolvio: `${guardName} (portería)`, conjunto }, override);
 ...
 */
exports.resolveVisitAuthorization = (0, https_1.onCall)({ cors: http_config_1.callableCorsOrigins, invoker: "public" }, async (request) => {
    const uid = request.auth?.uid;
    if (!uid)
        throw new https_1.HttpsError("unauthenticated", "Debes iniciar sesión.");
    const resultado = await (0, visita_no_anunciada_1.resolverAutorizacionDeVisita)(request.data, { uid, role: request.auth?.token?.role });
    if (resultado.aplicada) {
        const tenantId = request.data?.tenantId ?? "";
        const pase = await db.collection("visitorPasses").doc(request.data?.visitorPassId ?? "").get();
        const datos = pase.data();
        const otros = (await (0, visita_no_anunciada_1.residentesActivosDeLaUnidad)(tenantId, datos?.unitId ?? "")).filter((x) => x !== uid);
        if (otros.length > 0) {
            const [override, conjunto] = await Promise.all([
                getTenantNotificationOverride(tenantId, "visita_resuelta"),
                getTenantName(tenantId),
            ]);
            await deliverResidentNotifications("visita_resuelta", tenantId, otros, { visitante: datos?.visitorName ?? "", quienResolvio: resultado.resueltaPor, conjunto }, override);
        }
...
exports.createTrialWorkspace = (0, https_1.onCall)({ cors: http_config_1.callableCorsOrigins, invoker: "public", secrets: [email_1.resendApiKey] }, async (request) => {
    ...
    return (0, trial_workspace_1.atenderAltaDePrueba)(d, {
        ...
        cuentaConEseCorreo: async (email) => {
            const cuenta = await (0, auth_1.getAuth)().getUserByEmail(email).catch(() => null);
            if (!cuenta)
                return null;
            const perfil = (await db.collection("users").doc(cuenta.uid).get()).data();
            return { tenantId: typeof perfil?.tenantId === "string" ? perfil.tenantId : null };
        },
        crearAmbiente: trial_workspace_1.provisionTrialWorkspace,
...
exports.ensureReconciliationCases = (0, https_1.onCall)({ cors: http_config_1.callableCorsOrigins, invoker: "public" }, async (request) => {
    const uid = request.auth?.uid;
    if (!uid)
        throw new https_1.HttpsError("unauthenticated", "Debes iniciar sesión.");
    const resultado = await (0, conciliacion_casos_1.asegurarCasos)(request.data, uid, request.auth?.token?.role);
