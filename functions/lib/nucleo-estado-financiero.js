...
exports.esCodigoDeCuenta = esCodigoDeCuenta;
exports.compararCodigos = compararCodigos;
exports.aCentimos = aCentimos;
exports.esRecaudoDeCartera = esRecaudoDeCartera;
exports.construirEstadoFinanciero = construirEstadoFinanciero;
...
function esCodigoDeCuenta(clave) {
 ...
 */
function cajonDe(accountCode, category, plan, porDefecto) {
    if (accountCode && plan?.nombrePorCodigo.has(accountCode))
        return accountCode;
    if (category)
        return plan?.codigoPorSystemKey.get(category) ?? category;
    if (accountCode)
        return accountCode;
 ...
 */
function etiquetaDe(cajon, category, plan) {
 ...
 */
function construirEstadoFinanciero(entrada) {
    ...
    const acumular = (map, cajon, label, amount) => {
    ...
    const toRows = (map) => [...map.entries()]
        ...
        if (ca && cb)
            return compararCodigos(a.category, b.category);
        if (ca !== cb)
            return ca ? -1 : 1;
    ...
    const apertura = hayApertura ? openingBalance : 0;
    return {
        incomeByCategory,
        expenseByCategory,
        cuotaIncome,
 ...
 */
function sumarCuentasPorCobrar(cargos) {
    let total = 0;
    for (const cargo of cargos) {
        if (cargo.status === "cancelled" || cargo.status === "paid")
            continue;
 ...
 */
function sumarDeudaAProveedores(egresos) {
    let total = 0;
    for (const egreso of egresos) {
        if (egreso.status !== "registrado")
            continue;
