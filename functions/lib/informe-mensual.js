...
exports.PIE_DEL_INFORME = "Las firmas de este documento son constancia de quién lo emitió y quién lo aprobó dentro de Vivaru, " +
    ...
    return [
        {
            ...
            rows: i.income.map((l) => [l.label, formatearMonto(l.amount)]),
            total: ["Total de ingresos", formatearMonto(i.totalIncome)],
        },
        {
            title: "Egresos por cuenta",
            rows: i.expenses.map((l) => [l.label, formatearMonto(l.amount)]),
        ...
        },
        {
            ...
            rows: i.payables.byVendor.map((v) => [v.vendorName, formatearMonto(v.amount)]),
            total: [
