...
exports.construirEstadoDeCuenta = construirEstadoDeCuenta;
...
function fechaDe(c) {
    if (c.dueDate)
        return { fecha: c.dueDate, fuente: "dueDate" };
    if (c.period)
        return { fecha: c.period, fuente: "period" };
    return { fecha: String(c.createdAt ?? "").slice(0, 10), fuente: "createdAt" };
}
function construirEstadoDeCuenta(cargos, opts = {}) {
    ...
    const enRango = vigentes.filter((c) => {
        ...
        if (opts.desde && fecha < opts.desde.slice(0, fecha.length))
            return false;
        if (opts.hasta && fecha > opts.hasta.slice(0, fecha.length))
            return false;
        return true;
    });
    ...
    const ordenados = [...enRango].sort((a, b) => String(a.period).localeCompare(String(b.period)) ||
        fechaDe(a).fecha.localeCompare(fechaDe(b).fecha) ||
        String(a.id).localeCompare(String(b.id)));
    let acumulado = 0;
    const lineas = ordenados.map((c) => {
