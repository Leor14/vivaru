...
import { zonaDelConjunto } from "./zona-del-conjunto";
...
export function instantaneaDeUnInformeSellado(
  ...
  return {
    ...
    income: lista<LineaDelInforme>(d.income),
    expenses: lista<LineaDelInforme>(d.expenses),
 ...
 */
export function zonaParaPintarFechas(country: unknown): string {
  return zonaDelConjunto(country);
}

/** Las firmas tal como van al papel: nombre, cargo y fecha, escrita en la zona del conjunto. */
export function firmasParaElPdf(
  firmas: unknown,
  zona: string,
): { name: string; role: string; signedAt: string }[] {
  ...
  return (Array.isArray(firmas) ? firmas : []).map(
    (f: { name?: unknown; role?: unknown; signedAt?: { toDate?: () => Date } }) => {
