...
function dueDateFor(monthsAgo: number): string {
  const d = new Date();
  d.setDate(1);
...
export type TrialSeedResult = Record<string, number>;
...
export async function seedTrialWorkspace(tenantId: string, currency = "MXN"): Promise<TrialSeedResult> {
  ...
  const set = async (collection: string, docId: string, data: Record<string, unknown>) => {
  ...
  for (const [i, unit] of units.entries()) {
    await set("people", id(`person-${unit.slug}`), {
      fullName: `${NOMBRES[i % NOMBRES.length]} (${EXAMPLE_TAG})`,
  ...
  for (let m = FINANCE_MONTHS - 1; m >= 0; m--) {
    ...
    for (const [index, unit] of units.entries()) {
      ...
      await set("billingStatements", id(`bill-${period}-${unit.slug}`), {
        ...
        accountCode: cuentaParaConcepto("administracion").code,
        amount: MONTHLY_FEE,
        paymentAmount,
        balance,
  ...
  for (let m = FINANCE_MONTHS - 1; m >= 0; m--) {
    for (const g of gastos) {
      ...
      await set("expenses", id(`exp-${local}`), {
        category: g.category,
        accountCode: cuentaParaCategoriaDeEgreso(g.category).code,
