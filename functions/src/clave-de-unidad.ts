 ...
 */
export const CAMPO_ARCHIVADO_EN = "unitIdHuerfanoArchivadoEn";
export const CAMPO_ARCHIVADO_MOTIVO = "unitIdHuerfanoMotivo";

export function estaArchivado(datos: Record<string, unknown>): boolean {
  return Boolean(datos[CAMPO_ARCHIVADO_EN]);
}
 ...
 */
export const NO_ARCHIVABLES = new Set(["tenantUsers", "users"]);
 ...
 */
export function llevaDineroVivo(coleccion: string, datos: Record<string, unknown>): boolean {
  if (coleccion === "billingStatements") return typeof datos.balance === "number" && datos.balance > 0;
  if (coleccion === "advances") return typeof datos.remaining === "number" && datos.remaining > 0;
  return false;
}
