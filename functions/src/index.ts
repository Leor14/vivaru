...
import { esMiembroDelConjunto, MENSAJE_CUENTA_COMPARTIDA, tieneAccesoAOtrosConjuntos } from "./tenant-membership";
import { aplicarAccesoDeAdministrador } from "./acceso-de-administradores";
...
type UpdateTenantAdminInput = {
  uid: string;
  /** Ya no se usa: la edición no muda de conjunto (`PLAT-002` entrega 2). Se acepta y se ignora. */
  tenantId?: string;
  fullName: string;
  email: string;
  status: "active" | "inactive";
...
function normalizeCreateTenantAdminPayload(data: CreateTenantAdminInput) {
  ...
  const tenantIds = [
    ...new Set(
      [...(Array.isArray(data.tenantIds) ? data.tenantIds : []), data.tenantId]
...
export const createTenantAdmin = onCall<CreateTenantAdminInput>(
  ...
  try {
    ...
    if (faltante) {
      throw new HttpsError("not-found", `El conjunto ${faltante.id} no existe.`);
    }
    ...
    for (const id of data.tenantIds) {
      await assertBuzonAdmisible(id, data.email);
    }
    ...
    if (existingUser) {
      ...
      throw new HttpsError(
        "already-exists",
        "Ya existe una cuenta con ese correo. Si es de un administrador, edítalo en la lista para darle más conjuntos.",
      );
    }
    ...
    for (const id of data.tenantIds) {
      batch.set(
        db.collection("tenantUsers").doc(`${id}_${userRecord.uid}`),
        {
          uid: userRecord.uid,
          tenantId: id,
          fullName: data.fullName,
...
export const updateTenantAdmin = onCall<UpdateTenantAdminInput>(
  ...
  try {
    ...
    const conjuntoEspejo = normalizeText((perfilSnap.data() as { tenantId?: string }).tenantId);
    ...
    for (const membresia of membresias.docs) {
      const rol = (membresia.data() as { role?: string }).role;
      if (rol !== "tenant_admin" && rol !== "admin_tenant") continue;
      batch.set(
        membresia.ref,
        { fullName: data.fullName, email: data.email, status: data.status, updatedAt: now },
        { merge: true },
      );
    }
...
/**
 * `PRD-V-PLAT-002` entrega 2 (§16) — el superadmin da y quita acceso de administrador a
 * varios conjuntos, por persona. La lógica vive en `acceso-de-administradores.ts`; aquí
 * se exige superadmin (E2-R2), se expone el modo `simular` —lo que la consola pinta
 * como aviso antes de confirmar— y se deja rastro en CADA conjunto afectado (E2-R11).
 */
type SetTenantAdminAccessInput = {
  uid?: string;
  tenantIds?: unknown;
...
export const setTenantAdminAccess = onCall<SetTenantAdminAccessInput>(
  ...
  async (request) => {
    ...
    const tenantIds = Array.isArray(request.data?.tenantIds)
      ? (request.data.tenantIds as unknown[]).map((id) => normalizeText(typeof id === "string" ? id : "")).filter(Boolean)
      : null;
    if (!uid || !tenantIds) {
      throw new HttpsError("invalid-argument", "Faltan la persona o la lista de conjuntos.");
    }

    const resultado = await aplicarAccesoDeAdministrador({
      uid,
      tenantIds,
...
export const setOperationalUserStatus = onCall<SetOperationalUserStatusInput>(
  ...
  async (request) => {
    ...
    if (await tieneAccesoAOtrosConjuntos(targetUid, targetTenantId)) {
      throw new HttpsError("failed-precondition", MENSAJE_CUENTA_COMPARTIDA);
    }
 ...
 */
async function enviarAvisoDeCalendario(tenantId: string, clase: "aviso" | "vencidas"): Promise<boolean> {
  ...
  for (const d of cargos.docs) {
    const c = d.data() as { unitId?: string; balance?: number; status?: string };
    if (!c.unitId || (c.balance ?? 0) <= 0) continue;
    // El aviso mensual mira toda la deuda viva; el de vencidas, solo lo que ya venció.
    if (clase === "vencidas" && c.status !== "overdue") continue;
    unidades.add(c.unitId);
  }
  if (unidades.size === 0) return false;
  ...
  if (residentUids.length === 0) return false;

  await deliverResidentNotifications(clave as NotificationKey, tenantId, residentUids, { conjunto }, override);
  return true;
}
...
export const sendScheduledReminders = onSchedule({ schedule: "0 9 * * *", secrets: [resendApiKey] }, async () => {
  ...
  try {
    const r = await pasadaDeCalendarioDeCobranza(db, new Date(), enviarAvisoDeCalendario);
...
async function archiveBuffer(input: {
  tenantId: string;
  systemKey: "cartera_history" | "committee_reports" | "monthly_reports";
  fileName: string;
  ext: string;
  contentType: string;
  buffer: Buffer;
  ...
  category: string;
  /**
   * **Id fijo del documento, para que reintentar no duplique** (`FLOW-007` entrega 2).
   *
   * Sin él, el nombre del fichero lleva `Date.now()` y la fila de `documents` se
   * crea con `add()`: dos llamadas dejan dos PDF y dos filas. En el archivo
   * mensual eso da igual —corre una vez al mes y sin nadie mirando—, pero emitir
   * un informe puede reintentarse, y entonces el conjunto vería **dos informes de
   * marzo** sin saber cuál vale.
   *
   * Con el id fijo, la ruta de Storage también lo es, así que el segundo intento
   * **sobrescribe** en vez de acumular. Es el mismo «el id ES la clave» del paz y
   * salvo y del reparto.
   */
  stableId?: string;
}): Promise<string> {
...
export const monthlyFinancialArchive = onSchedule({ schedule: "0 6 1 * *", timeoutSeconds: 540, memory: "512MiB" }, async () => {
  ...
  for (const tDoc of tenants.docs) {
    ...
    const informeAnclado = await isFeatureEnabled("producto-informe-mensual", tenantId);
    ...
    let egresosDelConjunto: { amount?: number; status?: string; dueDate?: string; vendorId?: string; vendorName?: string }[] = [];
    let saldosDelConjunto: { openingBalance?: number }[] = [];
    if (informeAnclado) {
      ...
      saldosDelConjunto = saldosSnap.docs.map((d) => d.data() as { openingBalance?: number });
      egresosDelConjunto = egresosSnap.docs.map(
        (d) => d.data() as { amount?: number; status?: string; dueDate?: string; vendorId?: string; vendorName?: string },
      );
      ...
      saldoInicial = sumarSaldoDeApertura(saldosDelConjunto);
      porCobrar = sumarCuentasPorCobrar(stmts);
      deudaProveedores = sumarDeudaAProveedores(egresosDelConjunto);
    }
    ...
    await archiveBuffer({
      ...
      buffer: await buildSummaryPdf("Reporte de comité — Resumen mensual", `${prevMonth} · generado automáticamente`, [
        ...
        ...(informeAnclado
          ? [
              ["Saldo inicial del banco", estado.openingBalanceSource === "registrado" ? formatMoney(estado.openingBalance ?? 0) : "Sin saldo bancario de apertura"] as [string, string],
              ["Saldo final del fondo", formatMoney(estado.fundBalance)] as [string, string],
              ["Cuentas pendientes de cobro", formatMoney(estado.pendingReceivables)] as [string, string],
              ["Deuda a proveedores", formatMoney(estado.supplierDebt)] as [string, string],
            ]
          : []),
      ]),
      description: `Reporte de comité ${prevMonth} (automático, resumen financiero)`, source: "committee_report", sourceId: prevMonth, category: "reporte",
 ...
 */
async function descargarLogo(url: string | undefined): Promise<Buffer | undefined> {
...
async function identidadParaFirmar(tenantId: string, uid: string): Promise<{ name: string; role: string }> {
  const membership = (await assertTenantMember(tenantId, uid)) as {
    role?: string;
    status?: string;
    tenantId?: string;
    fullName?: string;
  ...
  if ((membership.status ?? "active") !== "active") {
    throw new HttpsError("failed-precondition", "Tu membresía en este conjunto está inactiva.");
  }
  const perfil = await db.collection("users").doc(uid).get();
  const nombre =
    (perfil.data() as { fullName?: string } | undefined)?.fullName ||
    membership.fullName ||
    "";
  if (!nombre) {
    ...
    throw new HttpsError(
      "failed-precondition",
      "Tu perfil no tiene nombre completo, y la firma lo lleva impreso. Complétalo antes de firmar.",
    );
  }
  return {
    name: nombre,
...
export const regenerateMonthlyReport = onCall<{ tenantId: string; period: string }>(
  { cors: callableCorsOrigins, invoker: "public" },
  async (request) => {
    const uid = request.auth?.uid;
    if (!uid) throw new HttpsError("unauthenticated", "Debes iniciar sesión.");
    ...
    if (!data?.tenantId || !data.period) {
      throw new HttpsError("invalid-argument", "Faltan el conjunto o el período.");
    }
    const tenantId = normalizeText(data.tenantId);
    ...
    await assertFeatureEnabled("producto-informe-mensual", tenantId);
    assertPeriodoValido(period);

    const instantanea = await leerYConstruirInstantanea(tenantId, period);
    const r = await guardarBorrador({ tenantId, period, instantanea, actorUid: uid });
    if (!r.escrito) {
      throw new HttpsError(
        "failed-precondition",
        r.motivo === "anulado"
    ...
    return { ok: true as const, reportId: idDelInforme(tenantId, period) };
  },
);

export const issueMonthlyReport = onCall<{ tenantId: string; period: string }>(
  { cors: callableCorsOrigins, invoker: "public", timeoutSeconds: 120, memory: "512MiB" },
  async (request) => {
    const uid = request.auth?.uid;
    if (!uid) throw new HttpsError("unauthenticated", "Debes iniciar sesión.");
    const data = request.data;
    ...
    const preparado = await prepararEmision({ tenantId, period });
    if (preparado.yaEmitido) {
      // Reintento idempotente: no se archiva un segundo PDF ni se vuelve a sellar.
      return { ok: true as const, reportId: idDelInforme(tenantId, period), created: false };
    }

    const tenantSnap = await db.collection("tenants").doc(tenantId).get();
    const tenant = tenantSnap.data() as { name?: string; branding?: { logoUrl?: string } } | undefined;
    ...
    const documentId = await archiveBuffer({
      ...
      stableId: `informe_${idDelInforme(tenantId, period)}`,
    });

    await sellarEmision({ tenantId, period, instantanea: preparado.instantanea, actorUid: uid, documentId });
    await writeAuditLog(tenantId, uid, "issue_monthly_report", { period, documentId });

    return { ok: true as const, reportId: idDelInforme(tenantId, period), created: true };
  },
);
...
export const signMonthlyReport = onCall<{ tenantId: string; reportId: string }>(
  ...
  async (request) => {
    ...
    if (!data?.tenantId || !data.reportId) {
      throw new HttpsError("invalid-argument", "Faltan el conjunto o el informe.");
    }
    const tenantId = normalizeText(data.tenantId);
...
export const voidMonthlyReport = onCall<{ tenantId: string; reportId: string; reason: string }>(
  { cors: callableCorsOrigins, invoker: "public" },
  async (request) => {
    const uid = request.auth?.uid;
    if (!uid) throw new HttpsError("unauthenticated", "Debes iniciar sesión.");
 ...
 */
export const switchActiveTenant = onCall<{ tenantId?: string }>(
  { cors: callableCorsOrigins, invoker: "public" },
  async (request) => {
    const uid = request.auth?.uid;
    if (!uid) throw new HttpsError("unauthenticated", "Debes iniciar sesión.");
