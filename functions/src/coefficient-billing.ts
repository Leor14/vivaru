...
export type LineaDeReparto = {
  unitId: string;
  unitLabel: string;
  /** El coeficiente con el que se calculó, congelado en el cargo. */
  coefficient: number;
  amount: number;
  /** La unidad monetaria extra que recibió del residuo, si la recibió. */
...
export type ResultadoReparto = {
  ...
  total: number;
  /** Suma de coeficientes de las unidades activas, para mostrar en el aviso. */
  coefficientSum: number;
};

export const TOLERANCIA_SUMA = 0.000001;
...
export function repartirPorCoeficiente(
  ...
  if (activas.length === 0) {
    throw new HttpsError("failed-precondition", "El conjunto no tiene unidades activas.");
  }
  ...
  const suma = activas.reduce((acc, u) => acc + (u.coefficient as number), 0);
  ...
  const sinResponsable = activas.filter(
    (u) => !u.billingResponsiblePersonId && !(Array.isArray(u.ownerIds) && u.ownerIds.length > 0),
  );
  if (sinResponsable.length > 0) {
    ...
    throw new HttpsError(
      "failed-precondition",
      `Estas unidades no tienen responsable ni propietario: ${nombres}${extra}.`,
    );
  }
  ...
  const exactos = activas.map((u) => (totalMinor * (u.coefficient as number)) / 100);
  ...
  const ajustes = new Array<number>(activas.length).fill(0);
  ...
  const lines: LineaDeReparto[] = activas.map((u, i) => ({
    unitId: u.id,
    unitLabel: u.unitLabel,
    coefficient: u.coefficient as number,
...
export type GenerarCorridaInput = {
  tenantId: string;
  /** Total del período a repartir entre las unidades activas. */
  totalAmount: number;
  /** `YYYY-MM`. */
  period: string;
  concept?: string;
  dueDate?: string;
  /** true = solo la vista previa; no se escribe nada (CA10/CA11). */
  dryRun?: boolean;
 ...
 */
export async function generarCorridaPorCoeficiente(
  input: GenerarCorridaInput,
  uid: string,
): Promise<GenerarCorridaResultado> {
  const firestore = db();
  ...
  const unidades: UnidadParaReparto[] = unitsSnap.docs.map((d) => {
    const data = d.data() as Omit<UnidadParaReparto, "id" | "unitLabel"> & { displayName?: string };
    return {
      id: d.id,
      unitLabel: (data as { displayName?: string }).displayName ?? d.id,
