...
import {
  buscarReferenciasAPersona,
  estaRegistrada,
  type ReferenciaEncontrada,
} from "./referencias-a-persona";
 ...
 */
async function assertPuedeGestionarPadron(role: unknown, uid: string | undefined, tenantId: string) {
  const rol = typeof role === "string" ? role : "";
  if (rol === "superadmin" || rol === "super_admin") return;
  if (!uid) throw new HttpsError("unauthenticated", "Debes autenticarte para gestionar el padrón.");
  const esAdmin = rol === "tenant_admin" || rol === "admin_tenant";
  if (!esAdmin || !(await esAdminActivoDelConjunto(tenantId, uid))) {
    throw new HttpsError("permission-denied", "No tienes permiso para gestionar el padrón de este conjunto.");
  }
...
function claveDelGrupo(ids: readonly string[]): string {
  return [...ids].sort().join("·");
}

export type FusionarPersonasInput = {
  tenantId: string;
  survivorId: string;
  mergedIds: string[];
  motivo: string;
...
export type FusionarPersonasResultado = {
  ok: true;
  fusionadas: number;
  repuntadas: number;
  decisionId: string;
...
export async function fusionarPersonas(
  input: FusionarPersonasInput,
  actor: { uid?: string; role?: unknown },
): Promise<FusionarPersonasResultado> {
  ...
  if (!tenantId || !survivorId) {
    throw new HttpsError("invalid-argument", "Falta el conjunto o la persona que se conserva.");
  }
  // `CF2`: fusionar sin elegir superviviente no es una fusión, es un borrado.
  if (mergedIds.length === 0) {
    throw new HttpsError("invalid-argument", "No hay ninguna persona que fusionar.");
  }
  if (mergedIds.includes(survivorId)) {
    throw new HttpsError("invalid-argument", "La persona que se conserva no puede estar entre las que se fusionan.");
  }
  if (!motivo) {
    throw new HttpsError("invalid-argument", "Escribe por qué son la misma persona. Sin motivo no se puede fusionar.");
  }
  ...
  const docs = snaps.map((s, i) => {
    if (!s.exists) {
      throw new HttpsError("not-found", `La persona ${refs[i].id} no existe.`);
    }
    const datos = s.data() as PersonaDoc;
    ...
    if (texto(datos.tenantId) !== tenantId) {
      throw new HttpsError("permission-denied", "Una de las personas pertenece a otro conjunto.");
    }
    if (datos.fusionadaEn) {
      throw new HttpsError("failed-precondition", `«${datos.fullName ?? s.id}» ya se fusionó antes.`);
    }
  ...
  if (cuentas.length > 1) {
    throw new HttpsError(
      "failed-precondition",
      `Estas personas tienen ${cuentas.length} cuentas de acceso distintas (${cuentas.join(", ")}). ` +
        "Retira el acceso de la que no se conserva antes de fusionar.",
    );
  }

  // ── 1. El barrido, ANTES de escribir nada (R4 / CF3) ──────────────────────
  const encontradas = await buscarReferenciasAPersona(firestore, tenantId, mergedIds);
  const desconocidas = encontradas.filter((r) => !estaRegistrada(r.coleccion, r.campo));
  if (desconocidas.length > 0) {
    const nombres = [...new Set(desconocidas.map((r) => `${r.coleccion}.${r.campo}`))].sort();
    throw new HttpsError(
      "failed-precondition",
      `Hay referencias que el inventario no conoce (${nombres.join(", ")}). ` +
  ...
  for (const grupo of agruparPorDocumento(encontradas)) {
    ...
    if (Object.keys(cambios).length > 0) await ref.update(cambios);

    for (const r of grupo.referencias.filter((x) => x.esLista)) {
      await ref.update({ [r.campo]: FieldValue.arrayRemove(r.personaId) });
      await ref.update({ [r.campo]: FieldValue.arrayUnion(survivorId) });
      repuntadas += 1;
    }
  }
  ...
  for (const id of mergedIds) {
    lote.set(
      firestore.collection("people").doc(id),
      {
        fusionadaEn: Timestamp.now(),
...
function agruparPorDocumento(referencias: readonly ReferenciaEncontrada[]) {
  const mapa = new Map<string, { coleccion: string; docId: string; referencias: ReferenciaEncontrada[] }>();
...
export type DescartarGrupoInput = {
  tenantId: string;
  ids: string[];
  motivo: string;
};
 ...
 */
export async function descartarGrupoDeDuplicados(
  input: DescartarGrupoInput,
  actor: { uid?: string; role?: unknown },
): Promise<{ ok: true; clave: string }> {
  const tenantId = texto(input?.tenantId);
  const motivo = texto(input?.motivo);
  const ids = [...new Set((input?.ids ?? []).map(texto).filter(Boolean))];

  if (!tenantId || ids.length < 2) {
    throw new HttpsError("invalid-argument", "Un grupo necesita al menos dos personas.");
  }
  if (!motivo) {
    throw new HttpsError("invalid-argument", "Escribe por qué no son la misma persona.");
  }
  ...
  for (const s of snaps) {
    const datos = s.data() as PersonaDoc | undefined;
...
function hash(valor: string): string {
  let h = 0;
  for (let i = 0; i < valor.length; i += 1) h = (Math.imul(31, h) + valor.charCodeAt(i)) | 0;
