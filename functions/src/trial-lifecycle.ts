...
export const RETENTION_DAYS_AFTER_EXPIRY = 60;
export const PURGE_WARNING_DAYS = 52; // avisar ~una semana antes de purgar
...
export type TrialLifecycleReport = {
  revisados: number;
  avisados: number;
  expirados: number;
  purgados: number;
  purgaPendiente: string[];
};
 ...
 */
export async function runTrialLifecycle(): Promise<TrialLifecycleReport> {
  const db = getFirestore();
  const report: TrialLifecycleReport = {
    revisados: 0,
    avisados: 0,
    expirados: 0,
    purgados: 0,
  ...
  for (const docSnap of trials.docs) {
    const data = docSnap.data() as { name?: string; trialEndsAt?: string; leadId?: string };
    ...
    if (left < 0) {
      await docSnap.ref.set(
        { status: "expired", expiredAt: new Date().toISOString(), updatedAt: Timestamp.now() },
        { merge: true },
      );
      report.expirados += 1;
      ...
      await notify(
        ...
        `${envTag()}[Trial vencido] ${data.name ?? docSnap.id}`,
        [
    ...
    if (REMINDER_DAYS.includes(left)) {
      const enviados = (docSnap.data() as { remindersSent?: number[] }).remindersSent ?? [];
      if (enviados.includes(left)) continue;

      await notify(
        docSnap.id,
        data.name ?? docSnap.id,
        `${envTag()}[Trial · ${left} día${left === 1 ? "" : "s"}] ${data.name ?? docSnap.id}`,
        [
  ...
  for (const docSnap of expired.docs) {
    const data = docSnap.data() as { name?: string; expiredAt?: string };
    if (!data.expiredAt) continue;
    const sinceExpiry = -(daysUntil(data.expiredAt) ?? 0);

    if (sinceExpiry >= RETENTION_DAYS_AFTER_EXPIRY) {
    ...
    if (sinceExpiry === PURGE_WARNING_DAYS) {
      await notify(
        docSnap.id,
        data.name ?? docSnap.id,
        `${envTag()}[Trial · purga próxima] ${data.name ?? docSnap.id}`,
        `El ambiente vencido de ${data.name ?? docSnap.id} cumple ${RETENTION_DAYS_AFTER_EXPIRY} días. Si no se convierte, sus datos se eliminarán.`,
      );
    }
  }
  ...
  async function notify(tenantId: string, tenantName: string, subject: string, body: string) {
    try {
...
export async function purgeTenant(tenantId: string): Promise<number> {
  const db = getFirestore();
  let deleted = 0;
