...
export function prometeFuturo(texto: string | null | undefined): string | null {
  ...
  return texto.match(COMPROMISO_FUTURO)?.[0] ?? null;
}
 ...
 */
export const CASOS_AUTOPRUEBA: ReadonlyArray<{ espera: "A" | "B" | "-"; texto: string }> = [
  // Positivos de A.
...
export function fallosDeAutoprueba(): string[] {
  const fallos: string[] = [];
  for (const { espera, texto } of CASOS_AUTOPRUEBA) {
