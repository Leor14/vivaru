...
export default function ResidentForcedPasswordChangePage() {
  ...
  const onSubmit = async (values: ForcedChangeForm) => {
    try {
      ...
      toast.success("Contraseña actualizada correctamente. Ya puedes usar el portal.");
      reset();
      if (process.env.NODE_ENV !== "production") {
  ...
  return (
    <section className="mx-auto w-full max-w-[36rem] py-3">
      <Card className="border-[var(--slate-200)] bg-white p-5 md:p-6">
        ...
        <CardTitle className="mt-3 text-xl">Debes actualizar tu contraseña para continuar</CardTitle>
        <CardDescription className="mt-2">
          Por seguridad, tu cuenta fue activada con una clave temporal. Ingresa esa clave y define una nueva contraseña personal.
        </CardDescription>

        <div className="mt-3 rounded-xl border border-[var(--slate-200)] bg-[var(--slate-50)] p-3 text-sm text-[var(--slate-700)]">
          Si tu acceso fue restablecido por administración, la clave temporal corresponde a tu número de documento.
        </div>
