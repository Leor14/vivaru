...
/**
 * Sube la evidencia de un ticket a `tenants/{tenantId}/support/{uid}/…` —o a
 * `support/equipo/…` cuando sube el equipo de Vivaru (`comoEquipo`, `PRD-V-FIX-005` · H3c)—.
 *
 ...
 * abriría la puerta a que ruta y firmante no coincidan. **El equipo es la
 * excepción**: su ruta viaja en el hilo que lee el conjunto, así que no lleva
 * su uid; `storage.rules` le deja escribir ahí porque `admin()` incluye al
 * superadmin, y la callable exige esa carpeta a las respuestas del equipo.
 *
...
export async function uploadSupportAttachments(
  ...
  for (const file of files) {
    ...
    const path = rutaDeAdjunto(tenantId, uid, opciones.comoEquipo === true, file.name, Date.now());
